...
| [`16_canal_demande_climatique.md`](16_canal_demande_climatique.md) | **Canal demande climatique (observation v0 — runtime d'observation livré, lot P3, PR #178/#215)** : grandeurs d'évapotranspiration de référence — **ET₀ journalière** (Hargreaves-Samani) et **VPD courant**, avec état qualitatif (`complet`/`degrade`/`indisponible`), exposées par `12_template_sensors/arrosage/demande_climatique.yaml`. Penman-Monteith **exclu** (ni vent ni rayonnement mesurés) ; **aucune constante inventée**. Complète `15` (ce que le climat **retire** vs ce qu'il **reste**) ; **n'émet aucune reco**, ne pilote rien, aucun seuil chiffré — **non branché** à aucune décision. |
| [`17_decision_v1.md`](17_decision_v1.md) | **Décision d'arrosage V1 (paramétrable, livrable)** : spécialisation **mono-station** de l'intention (`05`) — entrées (médiane/état réservoir sol, suspension pluie, fenêtre Arsenal, cooldown, plafond journalier, maître, préconditions/disponibilité pont, historique d'arrosage **fonctionnel**), **règle paramétrable** (besoin si médiane < seuil, hors suspension pluie, dans la fenêtre), **invariants de sûreté** (1/jour, cooldown, maître, fenêtre disjointe du secours, abstention ⇒ secours), **exécution déléguée** au script Run supervisé existant. **Aucune commande native nouvelle** ; observation = calibration, **pas** condition suspensive. Runtime V1 **livré** (PR #125, publié v16.3) ; extensions ultérieures (multi-zone, raffinements) non livrées. |
| [`18_notification_batterie.md`](18_notification_batterie.md) | **Notification mobile batterie faible Rain Bird (spécification, runtime non livré)** : architecture en **deux couches** (diagnostic d'interprétation + notification mobile), sources `battery_level` (principale, %) / `battery_voltage` (diagnostic), déclencheur futur `numeric_state below` avec **garde `from_state` numérique ≥ seuil** (anti-reload/anti-restart, reprise du modèle ScanWatch corrigé), **anti-répétition**, garde de **disponibilité du pont**, retour à la normale **non retenu** par défaut. **Aucune action d'arrosage** liée à la batterie ; seuils en **paramètres à arbitrer** ; **ID d'automation à fournir**. |
| [`19_modulation_duree.md`](19_modulation_duree.md) | **Modulation bornée de la durée d'arrosage — P4 (spécification ; runtime livré et branché, en validation runtime)** : transforme la **durée nominale** (helper opérateur, base réutilisée non écrasée) en **durée applicable** par une **composition ordonnée multiplicative** — précédence d'**abstention** puis `base × facteur_sol` (primaire, peut réduire/neutre/allonger sur signal **fiable qualifié**) `× facteur_climat` (secondaire, **allonge ou neutre, jamais réduit seul**), **arrondi au plus proche** (biais nul), **clamp absolu `[1,60]`**. **Canaux non fondus** (contributions séparées, exposées, neutralisables ; pas de score opaque), **abstention ≠ valeur `1,0` substituée**, **aucune réduction sur donnée dégradée**, **snapshot unique au Run**, **autorité de fin sur `arrosage_session_fin_prevue`**. **Bornes de modulation non inventées** : établies par exploitation des données puis **preuve runtime**. Cible = **branchement effectif** (pas d'observation à blanc). |
| [`20_arbitrage_pluie_besoin.md`](20_arbitrage_pluie_besoin.md) | **Arbitrage pluie ↔ besoin hydrique — couche réaction/décision pluie (spécification ; runtime de l'arbitrage non livré)** : définit le **verdict d'arbitrage riche** (vérité métier) pesant la **pluie observée**, l'**attente d'infiltration bornée**, la **pluie future** (crédibilité / quantité potentiellement utile / délai) et le **besoin hydrique établi** — six verdicts exclusifs (`aucune_suspension`, `attente_infiltration`, `report_pluie_credible`, `pluie_future_insuffisante`, `besoin_etabli_prioritaire`, `pluie_indecidable`). La **suspension booléenne** consommée par [`17`](17_decision_v1.md) en est une **projection dérivée**, jamais l'autorité. **La pluie ne fait que freiner** (jamais augmenter l'arrosage) ; donnée pluie non qualifiable ⇒ **indécidabilité, frein relâché**, jamais d'ordre d'arroser ni d'abstention générale. **Consomme par renvoi** la production ([`../meteo/pluie_production.md`](../meteo/pluie_production.md)) et l'honnêteté du cumul ([`06`](06_observation_et_preuves.md) §7) ; **aucune valeur inventée**. Cadrage : [`cadrage_arbitrage_pluie_besoin.md`](../../audits/02_conception/arrosage/cadrage_arbitrage_pluie_besoin.md) (C41). |

> **Deux natures de documents.** Les fichiers `01`–`07`, `09`–`11`, `13`, `14`, `15`,
> `16`, `17`, `18`, `19` et `20` sont **normatifs** (doctrine et invariants opposables). Les fichiers `08` et `12`
