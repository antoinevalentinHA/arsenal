...
| [`tendance_temperature.md`](tendance_temperature.md) | Tendance thermique (hausse/baisse/stable) des agrégats intérieurs — restitution Android Auto. **v1.2 (amendement)** : grandeur de décision v1.1 conforme au runtime (`moyenne_courte − moyenne_longue`) ; architecture amendée par l'ajout d'une couche de projection statistique ré-échantillonnée (§4.2), **runtime non conforme sur ce point, aucune passe runtime dans ce lot** (§19) ; validation terrain des seuils (§8.2) en dette ouverte, distincte. Voir l'audit fondateur : [`audit_tendance_temperature_sensibilite.md`](../../audits/01_rapports/meteo/audit_tendance_temperature_sensibilite.md) |
