...
- **Mécanisme historique déprécié** : `binary_sensor.presence_famille_securite_absent_depuis_5_min` (timer `timer.presence_famille_securite_absence` + automatisation `presence/absence_5_min.yaml`) n'est plus un déterminant décisionnel. Il est conservé en diagnostic jusqu'à sa suppression planifiée (phase 2).
