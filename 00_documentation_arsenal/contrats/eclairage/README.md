# ARSENAL — Contrats · Éclairage

Ce dossier regroupe les contrats de domaine et d'implémentation relatifs à
l'**éclairage**, tels qu'ils existent dans ce répertoire.

## Documents du domaine

| Document | Rôle |
|---|---|
| [`entree.md`](entree.md) | Contrat de domaine — éclairage Entrée |
| [`sejour.md`](sejour.md) | Contrat de domaine — éclairage Séjour |
| [`jardin.md`](jardin.md) | Contrat fonctionnel — éclairage jardin |
| [`garage.md`](garage.md) | Contrat d'implémentation du script autoritaire `script.garage_toggle` |
| [`recalage_nocturne_garage.md`](recalage_nocturne_garage.md) | Recalage nocturne multi-fenêtres — éclairage garage |
| [`mode_astronomie.md`](mode_astronomie.md) | Contrat transverse — mode astronomie (neutralisation séjour + jardin) |

## Navigation

- [Retour aux contrats](../README.md)
- [Index des contrats](../index.md)
- [Hub de navigation du domaine](../../navigation/domaines/eclairage.md)
