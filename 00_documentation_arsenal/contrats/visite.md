...
**Version :** 1.1.0  
**Statut :** Actif  
**Évolution :** assouplissement de l'isolation énergétique — exposition passive de `input_boolean.presence_visiteur` comme vérité d'occupation consommable en lecture par d'autres domaines (notamment ECS).
...
Le mode visite ne pilote aucun équipement énergétique. Aucune automation du domaine Visite n'écrit dans un actionneur énergétique, ne déclenche un cycle, ni n'influence une consigne.

Toutefois, l'état `input_boolean.presence_visiteur` constitue une **vérité d'occupation explicite** opposable, produite exclusivement par le domaine Visite dans le respect des invariants I3 et I4. 
À ce titre, il peut être consommé en lecture seule, sans rétroaction ni effet de bord vers le domaine Visite, par d'autres domaines comme contribution à leurs propres règles d'autorisation — notamment ECS pour la qualification d'occupation du bouclage.

La distinction est stricte : le domaine Visite **expose** un signal ; il ne **pilote** rien.

Cette exposition ne constitue pas une présence canonique au sens du contrat Présence maison et ne peut s'y substituer.

---

## 🧠 Capteur métier — Créneau visiteur actif

Fichier : 12_template_sensors/presence/visite.yaml
...
- écrire dans un actionneur ou une consigne d'un domaine énergétique,
- introduire de logique thermique.

La lecture passive de `input_boolean.presence_visiteur` par un autre domaine n'est **pas** un pilotage : elle est explicitement autorisée par I8.
...
- 12_template_sensors/presence/visite.yaml
...
### v1.1.0
- **§ En-tête** : ajout du bloc Version / Statut / Évolution (normalisation — le contrat n'était pas versionné jusqu'ici).
- **§ Invariants → I8 (Isolation énergétique)** : reformulation. L'interdiction de pilotage énergétique est préservée et précisée (aucune écriture, aucun déclenchement, aucune consigne). En complément, exposition explicite de `input_boolean.presence_visiteur` comme vérité d'occupation opposable consommable en lecture par d'autres domaines. Distinction *exposer* / *piloter* formalisée.
- **§ Interdictions → D4 (Pilotage énergétique interdit)** : reformulation. L'item ambigu « influencer le bouclage ECS » est remplacé par « écrire dans un actionneur ou une consigne du domaine ECS » pour lever toute ambiguïté avec la lecture passive autorisée par I8. Note explicite ajoutée que la lecture passive n'est pas un pilotage.
- **§ Changelog** : section créée (inexistante dans la version précédente du contrat).
- **Motivation** : permettre au domaine ECS de consommer `presence_visiteur` comme composante d'occupation maison stricte dans `binary_sensor.bouclage_autorise` (cf. contrat Bouclage v2.3.0), sans rompre la doctrine d'isolation énergétique du domaine Visite. Cas d'usage déclencheur : femme de ménage seule à la maison, qui consomme l'ECS sans qu'aucun `person.*` du foyer ne soit présent.

### v1.0.0
- Version initiale du contrat (non versionnée explicitement, normalisée a posteriori en v1.0.0 lors du bump v1.1.0).
