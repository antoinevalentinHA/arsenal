...
> **Actions (ouvert, P0 doctrine)** — la détection corrigée, les actions du mode panne restent **non contextualisées**, mais l'enjeu n'est pas un « drain batterie » global : l'architecture a **deux réservoirs** distincts — **UPS** (HA/box/réseau, *sobriété critique*) et **Bluetti** (chaîne thermique). L'**ECS haute / désinfection** est un **stockage thermique utile** sur le rail Bluetti (eau chaude + hygiène), **pas une anomalie** ; le **confort d'ambiance** est le seul volet sacrifiable. Manque : un **budget SOC Bluetti** et un **veto confort**. Audit métier : [`audit_actions_mode_panne_secteur.md`](../../../audits/01_rapports/pannes/audit_actions_mode_panne_secteur.md). **Sans patch runtime — doctrine à formaliser.**
