...
- Les coordonnées GPS de référence Arsenal (domicile) sont externalisées dans `secrets.yaml` via les clés `home_latitude` / `home_longitude`. Elles ne figurent jamais en clair dans le dépôt.
...
| Latitude              | *(non publiée — gérée par HA)* |
| Longitude             | *(non publiée — gérée par HA)* |
| Rayon                 | 824 m                   |
| Éditable UI           | Non                     |
...
| I5 | Les coordonnées GPS des zones métier Arsenal sont externalisées dans `secrets.yaml` via `latitude: !secret home_latitude` et `longitude: !secret home_longitude`. Les deux zones métier référencent **les mêmes clés secret**, garantissant une référence domicile unique sans exposer la valeur. Aucune coordonnée littérale ne figure dans le dépôt (contrat, checker, fixtures, commentaires inclus). |
...
| 1.0.4   | 2026-06-08 | I5 réécrit — coordonnées GPS externalisées via `!secret home_latitude`/`home_longitude` ; suppression de toute coordonnée littérale du contrat ; référence domicile unique garantie par clés secret partagées |