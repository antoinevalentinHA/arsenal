...
## Exception 9 — Sens de commande des volets (ouverture / fermeture)

### Objet

Dans les tuiles d'**action** du domaine Volets, la couleur identifie
visuellement le **sens de la commande** émise (ouverture / fermeture).
Elle ne porte **aucun jugement métier** (OK / KO / WARN / INFO) et ne
reflète **aucun état** du volet.

Cette distinction est nécessaire car les volets Bubendorf (via HomeKit)
n'exposent **aucun retour d'état fiable** et ne supportent que deux
commandes (`supported_features = 7` : ouvrir / fermer / position ;
**pas de stop**). La tuile ne peut donc traduire qu'une **intention de
commande**, jamais une position réelle. En l'absence d'état, distinguer
les deux commandes par la couleur améliore la lisibilité et réduit le
...
`shutter_close` de `shutter_action_base`, couche `10_action/` du domaine
...
« information ». Ces couleurs encodent exclusivement un **sens de
commande**, à l'image des modes HVAC.
