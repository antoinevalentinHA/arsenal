...
| `arsenal-ha-backup-timeline` | Pipeline NAS Synology | `arsenal_self` / `arsenal_nas` | C *(adapté — pipeline à la demande + tâches planifiées résiduelles, pas de matériel dédié)* | Publication MQTT NAS→HA (`arsenal/nas/audit/*`, `arsenal/nas/release_diff/*`) ; canal de commande HA→NAS (`arsenal/nas/admission/*`) livré et validé terrain pour `AUDIT`/`RELEASE_DIFF` (chantier C51) | Moyenne (patrimoine et observabilité, pas d'action matérielle directe) |
...
| **Méthode d'intégration** | Publication MQTT **NAS→HA** : `arsenal/nas/audit/state` (audit) ; `arsenal/nas/release_diff/state`\|`event` (release_diff). Canal de commande **HA→NAS** (`arsenal/nas/admission/command`\|`result`, vocabulaire fermé `AUDIT`/`RELEASE_DIFF`, admission, corrélation `request_id`↔`run_id`) livré et **validé terrain** (chantier C51, contrat [`contrats/nas_transactionnel.md`](../contrats/nas_transactionnel.md)) : `AUDIT` et `RELEASE_DIFF` sont désormais déclenchés à la demande depuis Arsenal/Home Assistant ; plus aucune tâche DSM périodique ne les déclenche. `Arsenal - Retention` (04:00, via le wrapper `run_retention.sh`) et `Arsenal - Quarantine Purge` (04:05) restent des tâches DSM planifiées, indépendantes de ce canal. |
