...
| [`arrosage`](domaines/arrosage.md) | C·Au·RT | Folder (18) — coexistence gouvernée Rain Bird (pont ESP32) + observation hydrique. **V1 automatique livrée** (décision/action mono-station, coexistence `rain_delay`) ; **observation v0 livrée** (canal réservoir sol). Chaîne d'audit partielle (conception). État → `REGISTRE_CHANTIERS` (C10). |
