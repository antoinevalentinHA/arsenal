...
- Couleurs : [`ui/couleurs/README.md`](../../ui/couleurs/README.md)
...
- Rapport navigation UI Lovelace (Révision v2) — [`audit_navigation_ui_lovelace.md`](../../audits/01_rapports/lovelace/audit_navigation_ui_lovelace.md) *(constat historique en lecture seule au HEAD `41bdc5c` ; leçon : résoudre les `!include` et surcharges d'instance avant tout constat de navigation. **Chantier clos** — P0 `/reglages-dashboard/maison` et P1 Retour `reglages/sommeil.yaml` corrigés ; voir §10 « Clôture du chantier » du rapport.)*
- Rapport ergonomie & découvrabilité UI (empirique) — [`audit_ergonomie_decouvrabilite_ui.md`](../../audits/01_rapports/lovelace/audit_ergonomie_decouvrabilite_ui.md) *(constat historique en lecture seule au HEAD `b90c6d0a` ; distinct de l'audit de conformité structurelle ci-dessus — porte sur l'expérience perçue (découvrabilité, affordance, cohérence commande/décision/diagnostic), pas sur la validité des `navigation_path`. Taxonomie A–E reprise sans modification ; 73/95 dashboards hors tuile Menu, sans orphelin architectural. Non arbitré, aucune correction engagée.)*
- Checker navigation Lovelace (R-LL-NAV-1) — `scripts/arsenal_contracts/check_lovelace_navigation_contracts.py` *(contrôle CI ; **résout les `!include` et surcharges d'instance** avant toute conclusion. Erreurs bloquantes : R1 clés de dashboard existantes, R2 cohérence du Retour, R3 culs-de-sac, **R4 segments de vue non canoniques** — forme canonique **stricte `/<dashboard-key>`**, les formes `/<dashboard-key>/<segment>` et `/<dashboard-key>/0` sont **interdites**. Warning non bloquant conservé : R5 (défauts latents des templates Paramètres/Diagnostics, surchargés partout — sans impact runtime). **Activé comme gate CI bloquant** via `contracts_lovelace_navigation.yml` (push + pull_request). Chantier navigation **clos** : R4 résorbé (73 → 0) puis durci en garde active contre toute régression.)*
