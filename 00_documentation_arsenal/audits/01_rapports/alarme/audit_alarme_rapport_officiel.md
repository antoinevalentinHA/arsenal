...
- **État** : Confirmé — **correctif implémenté, déployé et validé terrain** (CH-4-A, commit `5892d35`) : désarmement clavier → **un seul bip**, **aucun double bip**. **Constat soldé.**
...
>
> **Mise à jour (post-CH-4-B, dépôt `476116e`) — traçabilité.** Le **résidu runtime** d'`ALM-IMP-3` est **résolu** : l'automatisation morte `11_automations/alarme/sirene/stop.yaml` (ID `1002000000012`), l'entité fantôme `switch.sirene_alarm`, le `delay` latent et la notification morte sont **supprimés** (commit `476116e`). Le chemin d'arrêt réel est inchangé (`script.arret_sirene` = coupe immédiate ; durée device = auto-extinction reboot-safe). Subsiste une **dette de représentation seule** (documenter la durée device comme coupe-circuit canonique), à porter par **CH-5** / contrat `70`.
