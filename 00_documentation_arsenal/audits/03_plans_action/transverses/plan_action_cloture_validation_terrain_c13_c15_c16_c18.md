...
> **État d'avancement (2026-07-15).** **Instrumentation Recorder mergée par la PR #354 et activée au redémarrage complet de Home Assistant le 2026-07-13 (T0 confirmé).** Échéances de réévaluation effectives : C16 **2026-09-11** ; C13 **2026-11-10**. **C18 clôturé** (validation live T1 + T5 du 2026-07-13 ; registre ⑤). **C15 clôturé** (test live contrôlé du 2026-07-15 : reboot pendant un cycle lave-vaisselle réel → re-création de la persistante à +47 s, confondant de re-détection exclu par les données, contrefactuel pré-correctif présent ; registre ⑤ — [preuve](../../01_rapports/notifications/preuve_terrain_c15_survie_persistantes_reboot.md)). Restent ouverts : **C16** (attente prochaine pluie), **C13** (attente occurrence réelle). **Les deux clôtures sans instrumentation prévues au §6 sont donc acquises** ; le reliquat du plan ne dépend plus que d'occurrences naturelles.
...
| **C15** ✅ **CLOS (2026-07-15)** | reboot pendant état actif → **recréation** de la persistante | **Live contrôlé** (reboot pendant un **vrai cycle** électroménager) — *réalisé* | **NON** (test live direct ; `call_service` create déjà captés) — *aucune instrumentation n'a été nécessaire, comme prévu* | *observation* : `input_boolean.lave_vaisselle_cycle` **ou** `input_boolean.buanderie_cycle` (actif) ; notif `lave_vaisselle_cycle`/`buanderie_cycle` ; auto `10080000000006`/`10080000000007` ; jalon `systeme_stable` | Cycle réel actif → redémarrage HA → après `systeme_stable→on`, la notification persistante **réapparaît** (observée en UI), flag encore actif — **✅ SATISFAIT** : cycle `on` à 21:09:29 (aucun retrait), reboot 22:01:30, `create` 22:02:17 (**+47 s** ≈ délai `systeme_stable` 45 s), flag encore actif à la sauvegarde (22:22). **Confondant de re-détection exclu** : aucun `turn_on` entre reboot et `create` ⇒ flag *restauré*, pas redétecté (`debut.yaml` = 30 W / 3 min, incompatible avec +47 s). **Contrefactuel** dans la même fenêtre : 7 reboots pré-correctif cycle actif → **aucun acte**. [Preuve](../../01_rapports/notifications/preuve_terrain_c15_survie_persistantes_reboot.md) | — |
...
2. ~~**C15 — test live contrôlé** — immédiat, zéro instrumentation. Reboot pendant un vrai cycle électroménager, vérifier la réapparition de la persistante. Clôture C15.~~ **✅ FAIT le 2026-07-15 — C15 clos** ([preuve](../../01_rapports/notifications/preuve_terrain_c15_survie_persistantes_reboot.md)).
