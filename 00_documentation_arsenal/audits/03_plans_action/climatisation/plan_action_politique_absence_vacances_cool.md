...
|---|---|---|---|---|
| **C20-L1** | **Contrat** | Politique d'absence COOL : veto immédiat Vacances ; durée gouvernée (bornes/granularité) ; continuité physique (horodatage + échéance recalculée, boot-proof) ; vérité composite `(absence_longue OR vacances)` ; fail-closed. **→ Livré (rédigé, en revue) 2026-07-14** : [`15_absence_vacances_veto_cool.md`](../../../contrats/climatisation/15_absence_vacances_veto_cool.md) + réconciliations (04, 06, capteurs blocages/autorisations/decision, index). Points validables au stop point : bornes helper (8/48/1), retrait décisionnel du timer, nom du checker Lot 2. | D1,D3,D4,D5,D6,D7 | contrat rédigé, revu, opposable |
| **C20-L2** | **Checkers** | Gate(s) de la politique : présence de la cause Vacances dans l'autorisation ; fail-closed ; (rappel : `initial` couvert par CI existante `contracts_initial_key.yml`). **→ Livré 2026-07-14** : oracle structurel `tools/arsenal_ci/behavior/cool_veto_gate.py` (INV-VETO-2/3/4/5/6/7/9) + `tests/test_lot_5_1_cool_veto_absence_vacances.py` (fixtures pré/post, 16 tests). Vert en pytest `arsenal_ci` (162). **Câblage retenu (arbitrage propriétaire)** : pytest arsenal_ci, **sans** workflow `contracts_*` bloquant ; le **gate live** contre le runtime réel est branché au **Lot 3** (sinon rouge avant implémentation). | D1,D3 | checker vert, mutation vérifiée |
| **C20-L3** | **Runtime** | Helper de durée (sans `initial:`, repli `float`) ; horodatage de début d'absence + échéance recalculée sur état ; vérité composite calculée ; extension `sensor.clim_raison_decision`/`sensor.clim_verdict_cool` (causes) ; câblage dans `binary_sensor.autorisation_clim_cool`. **→ Livré (en revue) 2026-07-14** : `03_input_numbers/climatisation/absence/duree.yaml`, `07_input_datetimes/climatisation/absence.yaml`, automation `10030000000122` (`horodatage_absence.yaml`), `blocages/veto_absence_vacances.yaml`, `blocages/absence_longue.yaml` (recomputé), `autorisation/cool.yaml`, `decision/raison.yaml` + `verdict_mode.yaml` ; **timer `absence_longue_clim` + automation `10030000000104` retirés**. Gate live : `test_lot_5_2_cool_veto_runtime.py` + workflow `arsenal-ci-climatisation.yml` (registre couverture 87). | D1–D7 | runtime conforme au contrat, CI verte |
| **C20-L4** | **Dashboard** | Exposition du réglage de durée (section « ❄️ Mode Cool », pattern `tile`+`numeric-input`) ; UI non décisionnaire. **→ Livré (en revue) 2026-07-14** : sous-section « ⏳ Absence longue » + tuile `input_number.clim_duree_absence_longue` dans `18_lovelace/dashboards/climatisation/reglages.yaml` (affichage seul ; checkers lovelace verts). | D3 | réglage visible, aucune logique en UI |
| **C20-L5** | **Validation terrain** | S1–S12 (§4). **→ Protocole ouvert 2026-07-14** : [`protocole_validation_terrain_absence_cool.md`](../../04_chantiers/climatisation/protocole_validation_terrain_absence_cool.md) (12 scénarios, lecture seule, trace à remplir). **Aucune preuve encore recueillie — C20 non clôturable.** | — | trace de validation remplie |
