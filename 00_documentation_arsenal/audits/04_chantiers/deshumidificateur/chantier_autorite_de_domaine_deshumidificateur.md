...
| **Statut** | **CLOS (2026-07-26) — terrain validé.** Ouverture → cadrage (D1–D10) → contrat [`autorite_de_domaine.md`](../../../contrats/deshumidificateur/autorite_de_domaine.md) → échafaudage → **bascule** (décision auto publiée `deshumidificateur_decision_auto` ; `etat_commande` raffiné ; `activation`/`desactivation` = producteurs ; **application** = consommateur exécutoire unique ; **convergence boot** + `converger_auto` ; retry souverain) → **UI** « Autorité & reprise en main » (porteur d'intention + 2 automations de médiation ; cartes `15_autorite/` ; dashboard à affichage conditionnel) → **retrait physique switchbot** (branches `is_deshumidificateur` + helpers dormants supprimés ; `switchbot_transactionnel.md` v2.1.0 ; invariant I1 migré). **Garde CI `R-CALL-DESHUM`** : numerus clausus {application, retry_on, retry_off} + anti-routage + interdiction d'appel direct. Acquis : pivot **OUI**, min-on/min-off = politiques non opposables au manuel (preuve corpus), écrivain unique `set_deshumidificateur_state`. |
...
| **Prochain jalon** | **— (chantier clos).** Terrain validé ; tous les livrables (échafaudage, bascule, UI, retrait physique switchbot, garde CI) intégrés. Extensions futures hors périmètre : expiration volontaire (INV-AUT-7), requalification matérielle min-on/min-off (sur preuve). |
