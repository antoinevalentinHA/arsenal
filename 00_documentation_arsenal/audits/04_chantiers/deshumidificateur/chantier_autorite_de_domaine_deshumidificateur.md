...
| **Statut** | **ACTIF (2026-07-26) — ÉCHAFAUDAGE + BASCULE + UI LIVRÉS ; validation terrain à venir.** Ouverture → cadrage (D1–D10) → contrat [`autorite_de_domaine.md`](../../../contrats/deshumidificateur/autorite_de_domaine.md) → échafaudage → **bascule** (décision auto publiée `deshumidificateur_decision_auto` ; `etat_commande` raffiné ; `activation`/`desactivation` = producteurs ; **application** = consommateur exécutoire unique ; **convergence boot** + `converger_auto` ; retry souverain ; garde CI `R-CALL-DESHUM`) → **UI** : section « Autorité & reprise en main » (porteur d'intention `deshumidificateur_autorite_intention` + 2 automations de médiation exécution/synchro ; cartes `15_autorite/` ; dashboard `principal.yaml` à affichage conditionnel). Retrait physique switchbot **déféré** (PR C40 dédiée). Acquis : pivot **OUI**, min-on/min-off = politiques non opposables au manuel (preuve corpus), écrivain canonique `set_deshumidificateur_state`. |
...
| **Prochain jalon** | **Validation terrain** (comportement boot/restitution, convergence, sélecteur d'autorité, affichage conditionnel). En parallèle, **PR C40 dédiée** : retrait physique du support déshum de `bot_transaction_execute` + révision `switchbot_transactionnel.md`. Puis clôture. |
