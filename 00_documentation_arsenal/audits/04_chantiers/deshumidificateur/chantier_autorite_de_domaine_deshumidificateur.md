...
| **Statut** | **ACTIF (2026-07-26) — ÉCHAFAUDAGE + BASCULE LIVRÉS ; UI à venir.** Ouverture → cadrage (D1–D10) → contrat [`autorite_de_domaine.md`](../../../contrats/deshumidificateur/autorite_de_domaine.md) → échafaudage → **bascule** : décision auto publiée `input_select.deshumidificateur_decision_auto` ; `sensor.deshumidificateur_etat_commande` raffiné (auto = `decision_auto`, manuel = consigne ; `demarrage_recommande` = théorique) ; `activation`/`desactivation` = **producteurs de décision** (n'appellent plus l'écrivain) ; **application** = consommateur exécutoire unique (`queued` sans perte, relecture live, impulsion unique état-OU-événement) ; **convergence boot** + `converger_auto` (absorbe `reconciliation_demarrage`) ; retry souverain ; **garde CI `R-CALL-DESHUM`** (numerus clausus + anti-routage). Retrait physique switchbot **déféré** (PR C40 dédiée). Acquis : pivot **OUI**, min-on/min-off = politiques non opposables au manuel (preuve corpus), écrivain canonique `set_deshumidificateur_state`. |
...
| **Prochain jalon** | **Passe UI** : section « Autorité & reprise en main » (sélecteur d'autorité d'intention + affichage conditionnel ; manuel → `{marche, arrêt}` + décision exécutoire ; auto → décision + diagnostic). En parallèle, **PR C40 dédiée** : retrait physique du support déshum de `bot_transaction_execute` + révision `switchbot_transactionnel.md`. Puis validation terrain → clôture. |
