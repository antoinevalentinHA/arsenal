...
| **Statut** | **ACTIF (2026-07-26) — ÉCHAFAUDAGE LIVRÉ (inerte) ; bascule + UI à venir.** Ouverture → cadrage (D1–D10) → contrat [`autorite_de_domaine.md`](../../../contrats/deshumidificateur/autorite_de_domaine.md) → **échafaudage inerte** : porteurs `input_select.deshumidificateur_titulaire_autorite` + `…_consigne_manuelle` (`{on,off}`, sans `initial:`), décision exécutoire dérivée `sensor.deshumidificateur_etat_commande` (anti-fallback via `availability` ; auto = `demarrage_recommande`, manuel = consigne), primitives `script.deshumidificateur_entrer_mode_manuel` / `…_revenir_mode_automatique`. **Inerte** : rien ne consomme encore l'exécutoire (pipeline existant en vigueur). Acquis : pivot **OUI**, min-on/min-off = **politiques non opposables au manuel** (preuve corpus), écrivain canonique `set_deshumidificateur_state`, support déshum de `bot_transaction_execute` à retirer (bascule). |
...
| **Prochain jalon** | **Passe bascule** : automation d'application = consommateur exécutoire unique de `sensor.deshumidificateur_etat_commande` ; `activation`/`desactivation` → producteurs de décision (n'appellent plus l'écrivain) ; retry souverain ; **retrait du support déshum de `bot_transaction_execute`** + garde CI ; numerus clausus d'appelants ; iso-comportement auto. Puis UI (patron autorité d'intention) → validation terrain → clôture. |
