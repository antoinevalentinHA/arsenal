...
| **Statut** | **⏸️ PARQUÉ (2026-07-25) — ÉCHAFAUDAGE INERTE LIVRÉ, BASCULE EN ATTENTE DE C30.** §5.1 = **OUI** ; D1–D8 actées ; **contrat** [`16_autorite_de_domaine_climatisation.md`](../../../contrats/climatisation/16_autorite_de_domaine_climatisation.md) (§16.1–§16.8) + précisions 03 (v1.4)/07 (v1.4)/09 (v1.6)/06 (§10)/15 (v1.1) ; **échafaudage runtime inerte** (patron VMC #574) : porteurs `clim_titulaire_autorite` / `clim_consigne_manuelle` (sans `initial:`), décision exécutoire dérivée `sensor.clim_mode_commande` (anti-fallback), primitives `clim_entrer_mode_manuel` / `clim_revenir_mode_automatique`. **Inerte** : rien ne consomme encore `clim_mode_commande` (l'application lit toujours `clim_target_mode`) → **aucun changement de comportement, mode manuel pas encore exécutoire** ; aucune automation ajoutée. **Parqué** car la **bascule** est atomique et **gated C30** (P1), lui-même terrain-bloqué (occurrence *fail-open* non datable). |
...
| **Déclencheur de réveil** | **C30 stabilisé** — preuve terrain *fail-open* obtenue et couche conformité/exécution qualifiée. Alors : **bascule** (rebrancher application puis conformité/Watchdog sur `sensor.clim_mode_commande` → mode manuel exécutoire) → **UI** de reprise en main → validation terrain → clôture fonctionnelle. Indépendant de C30, mobilisable à tout moment : item de sûreté **étage 1 compresseur** (cadrage §3). **STOP-avant-écriture** maintenu à chaque étape. |
| **Registre** | Chantier **C37** — ② Parqués (réveil = C30 stabilisé), cf. [`REGISTRE_CHANTIERS.md`](../../REGISTRE_CHANTIERS.md). Branche **climatisation** de `D-C36-L4` ; la branche **chauffage** reste dormante en ③. **Ce document est la source faisant foi pointée par la ligne.** |
