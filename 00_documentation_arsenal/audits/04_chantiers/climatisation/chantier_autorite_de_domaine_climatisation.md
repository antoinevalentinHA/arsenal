...
| **Statut** | **ACTIF (2026-07-25) — CONTRAT LIVRÉ.** §5.1 = **OUI** ; décisions D1–D8 actées (cadrage [`cadrage_autorite_de_domaine_mode_manuel_climatisation.md`](../../02_conception/climatisation/cadrage_autorite_de_domaine_mode_manuel_climatisation.md)) ; **contrat écrit** : nouveau [`16_autorite_de_domaine_climatisation.md`](../../../contrats/climatisation/16_autorite_de_domaine_climatisation.md) (§16.1–§16.8, patron VMC) + précisions chirurgicales des invariants de souveraineté dans 03 (v1.4), 07 (v1.4), 09 (v1.6), 06 (§10), 15 (v1.1, INV-VETO-10). **Aucun runtime modifié** ; le rebranchement conformité/Watchdog est spécifié comme **cible contractuelle**, son runtime **coordonné avec C30**. |
...
| **Prochain jalon** | **Passe runtime** — porteurs (titulaire, consigne mode), décision exécutoire dérivée anti-fallback, primitives supervisées d'entrée/retour, sur le patron VMC ; **le rebranchement conformité/Watchdog est coordonné avec C30** (P1), le reste ne l'est pas. Puis UI → validation terrain → clôture fonctionnelle. Item de sûreté **étage 1 compresseur** à arbitrer (cadrage §3). **STOP-avant-écriture** maintenu à chaque étape. |
