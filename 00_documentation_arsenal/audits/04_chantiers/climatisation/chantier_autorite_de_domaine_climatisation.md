...
| **Statut** | **ACTIF (2026-07-25) — CONTRAT + ÉCHAFAUDAGE RUNTIME INERTE LIVRÉS.** §5.1 = **OUI** ; D1–D8 actées ; **contrat** [`16_autorite_de_domaine_climatisation.md`](../../../contrats/climatisation/16_autorite_de_domaine_climatisation.md) (§16.1–§16.8) + précisions 03 (v1.4)/07 (v1.4)/09 (v1.6)/06 (§10)/15 (v1.1). **Échafaudage runtime inerte livré** (patron VMC #574) : porteurs `clim_titulaire_autorite` / `clim_consigne_manuelle` (sans `initial:`), décision exécutoire dérivée `sensor.clim_mode_commande` (anti-fallback), primitives `clim_entrer_mode_manuel` / `clim_revenir_mode_automatique`. **Inerte** : aucun consommateur d'exécution ne lit encore `clim_mode_commande` (l'application lit toujours `clim_target_mode`) → **aucun changement de comportement, mode manuel pas encore exécutoire**. Aucune automation ajoutée. |
...
| **Prochain jalon** | **Bascule (coordonnée C30)** — rebrancher l'**application** puis la **conformité/Watchdog** sur `sensor.clim_mode_commande` (rend le mode manuel exécutoire) ; cette couche est celle que C30 (P1) durcit → **coordination obligatoire**. Puis **UI** de reprise en main (appelant les primitives) → validation terrain → clôture fonctionnelle. Item de sûreté **étage 1 compresseur** à arbitrer (cadrage §3). **STOP-avant-écriture** maintenu à chaque étape. |
