...
| **Statut** | **ACTIF (2026-07-25) — ÉCHAFAUDAGE LIVRÉ (inerte) ; bascule + UI à venir.** Ouverture → cadrage (D1–D9) → contrat [`85_autorite_de_domaine_chauffage.md`](../../../contrats/chauffage/85_autorite_de_domaine_chauffage.md) → **échafaudage inerte** : porteurs `input_select.chauffage_titulaire_autorite` + `…_consigne_manuelle` (sans `initial:`), décision exécutoire dérivée `sensor.chauffage_mode_commande` (anti-fallback via `availability` ; auto = `mode_session`, manuel = consigne), primitives `script.chauffage_entrer_mode_manuel` / `…_revenir_mode_automatique`. **Inerte sur l'exécution** : rien ne consomme encore la décision exécutoire (pipeline existant en vigueur). **Atout** : écrivain d'exécution **déjà unique** (CH-4/R-CALL-1), pas de watchdog à démanteler, **bascule non gated** (pas de C30). |
...
| **Prochain jalon** | **Passe bascule** : brancher l'application + la conformité transactionnelle (retry) sur la décision exécutoire `sensor.chauffage_mode_commande` ; **migrer l'override** `mode_confort_chauffage` vers le modèle titulaire ; **étendre le numerus clausus CH-4** à la primitive manuelle ; démontrer l'**iso-comportement en auto** (`chauffage_mode_commande == mode_session`). Puis UI (patron d'autorité d'intention + affichage conditionnel) → validation terrain → clôture fonctionnelle. |
