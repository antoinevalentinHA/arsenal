...
| **Prochain jalon** | **Passe UI** : section « Autorité & reprise en main » sur le patron d'autorité d'intention + affichage conditionnel (moule VMC/clim) — sélecteur d'autorité ; manuel → régime `{confort, réduit}` + décision exécutoire ; auto → décision + diagnostic. Puis **validation terrain** (les deux régimes, prise/commande/restitution, convergence boot, override migré) → clôture fonctionnelle. |
