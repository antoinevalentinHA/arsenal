...
| **Statut** | **ACTIF (2026-07-25) — CONTRAT LIVRÉ (cible), runtime à venir.** Ouverture → cadrage (D1–D9, arbitrages tranchés : pivot **OUI**, surface **régime `{confort, réduit}`**, portée **domaine entier** VP8 préservé, override **englobé**, blocages **A/B**, anti-court-cycle par **dérogation**) → **contrat** [`85_autorite_de_domaine_chauffage.md`](../../../contrats/chauffage/85_autorite_de_domaine_chauffage.md) (§85.1–§85.8, patron VMC/clim §16). **Atout confirmé** : écrivain d'exécution **déjà unique** (CH-4/R-CALL-1), **pas de watchdog ré-asserteur** à démanteler, **aucune dépendance de séquencement type C30** (bascule non gated). **Runtime/UI non encore livrés.** |
...
| **Prochain jalon** | **Passe runtime** : échafaudage (titulaire `input_select.chauffage_titulaire_autorite`, consigne manuelle, décision exécutoire dérivée anti-fallback, primitives, porteur d'intention + automations de médiation) **puis bascule** (application + conformité transactionnelle sur l'exécutoire ; override `mode_confort_chauffage` migré ; numerus clausus CH-4 étendu à la primitive manuelle ; iso-comportement en auto). Puis UI (patron d'autorité d'intention + affichage conditionnel) → validation terrain → clôture fonctionnelle. |
