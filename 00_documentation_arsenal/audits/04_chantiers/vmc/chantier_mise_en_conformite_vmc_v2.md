...
| **Prochain jalon** | **L5 — acquisition de la référence terrain**, **jalon actif, en partie servi** (§5). Méthode tranchée : **caractérisation statistique seule**, sans étiquetage. **Référence terrain partielle établie dans `arsenal-runtime` et synthèse préparée pour intégration dans Arsenal** — [`reference_terrain_partielle_vmc.md`](reference_terrain_partielle_vmc.md). **L5 n'est pas soldé** : restent à instruire le débit physique, la corroboration décision → commande → relais, le mécanisme de libération et les arbitrages préalables à L2b. **L2b (calibration finale) reste ordonnancé après L5** et **n'est pas activé** ; **aucune correction runtime n'est autorisée tant que la séquence probatoire et L2b ne sont pas soldées**. |
