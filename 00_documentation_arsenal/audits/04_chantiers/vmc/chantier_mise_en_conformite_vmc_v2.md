...
| **Domaine** | VMC. |
| **Statut** | **Ouvert — Lots 1, 2c et 3 intégrés ; L2a acquis ; L4 conclu — préparé, en attente de commit et de merge.** `vmc.md` **v2.1** est normatif. **Deux arbitrages propriétaires (2026-07-21)** : (1) l'effet de la haute vitesse est **acquis par l'usage** — mesure du débit et identification matérielle **hors dispositif** ; (2) la trace `input_boolean.vmc_haute_vitesse` est **suffisamment autoritative par construction** — **L4 conclut « moyens suffisants »**, aucune exposition diagnostique, aucune modification de `recorder.yaml`. La frontière de libération étant un **niveau** et non une cinétique, « suffisamment assaini » devient **calibrable sur l'historique existant** : plus aucune preuve n'attend d'épisode en haute vitesse. Aucune calibration n'est pour autant acquise. Aucun runtime, UI ni checker modifié. |
...
| **Prochain jalon** | **L4 — conclusion « moyens suffisants »**, portée par le présent lot (en attente de merge) ; **L5 deviendra le jalon actif après intégration**. **L2b (calibration finale) reste ordonnancé après L5** ; **aucune correction runtime n'est autorisée tant que la séquence probatoire et L2b ne sont pas soldées**. |
...
| **L4** | **Audit des moyens d'observation** — **conclusion : moyens suffisants** (2026-07-21). La trace est jugée suffisamment autoritative par construction — **préparé, en attente de merge** | Aucune exposition diagnostique, aucune modification de `recorder.yaml`, aucune instrumentation, aucune mesure matérielle |
| **L5** | Acquisition d'une référence **avant** changement — **allégée** : plus d'attente d'un épisode en haute vitesse (§3.E du protocole) | Sans référence, l'effet du changement ne sera pas mesurable |
