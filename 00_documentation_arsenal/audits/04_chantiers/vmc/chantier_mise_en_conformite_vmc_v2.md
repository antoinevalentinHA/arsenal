...
| **Prochain jalon** | **L5 — acquisition de la référence terrain** (§5). Le corpus est acquis ; reste à trancher la méthode de caractérisation des épisodes (étiquetage prospectif ou caractérisation statistique). **L2b (calibration finale) reste ordonnancé après L5** ; **aucune correction runtime n'est autorisée tant que la séquence probatoire et L2b ne sont pas soldées**. |
...
| **L5** | Acquisition d'une référence **avant** changement — **allégée** (§3.E) et **corpus déjà acquis** : 38 sauvegardes, 199,6 j, dans `arsenal-runtime/snapshots/` | Sans référence, l'effet du changement ne sera pas mesurable |
