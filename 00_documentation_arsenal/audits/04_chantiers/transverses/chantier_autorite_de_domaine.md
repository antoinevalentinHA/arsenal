...
| **Statut** | **Ouvert — doctrine (#571), amendement VMC v2.5 (#572) et contrat v2.6 (#573) livrés ; conception runtime validée. PR B : échafaudage inerte de l'autorité VMC + correction `availability` de la cohérence physique.** Le mode manuel reste **NON EXÉCUTOIRE jusqu'à PR C** (bascule) ; PR B sans changement de `gestion_auto` ni de conformité, sans UI, sans nouvel ID d'automatisation. |
...
| **Prochain jalon** | Doctrine (#571), amendement v2.5 (#572), contrat v2.6 (#573) **livrés**. **PR B (présente)** : échafaudage **inerte** + `availability` cohérence — `input_select.vmc_titulaire_autorite` / `vmc_consigne_manuelle`, `binary_sensor.vmc_haute_vitesse_commandee` (dérivé pur, anti-fallback), primitives `script.vmc_entrer_mode_manuel` / `vmc_revenir_mode_automatique`. **PR C** : bascule L4+L6 (application consomme la décision exécutoire, conformité vs décision exécutoire) + tests terrain — **active** le mode manuel. |
