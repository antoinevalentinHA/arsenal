...
| **Statut** | **CLOS — conforme (2026-07-19).** Arc complet : **arbitrage** (Lot 1) → **contrats** 2A (#413) / reconsolidation (#414) / 2B (#415) → **agrégats abstinents** (R2 #420) → **défense fonctionnelle complète** (C28 L3A #418 + L3B #419 + R2 #420) → **conformité de restitution finale (#422)** : frontière basse publiée, catégories backend O3 MIN/MAX, cartes « backend décide / UI restitue » (aucune lecture de helper, aucun recalcul UI, aucun cross-entity), palette Exception 2 étendue, gris indispo prioritaire. **Les six écarts opposables du Lot 2B sont résolus (#422)** ; checker **T4** + mutation-tests + **CI verts** ; implantation dashboard inchangée. |
