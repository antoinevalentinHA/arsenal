...
| Météo / palmarès | 9 | sensors météo, palmarès, recorder | lexicale (palmarès chaud : partiellement structurelle) | consolidation/stabilisation/HR/jardin : versions citées sans chemin ; **palmarès chaud/froid : contrats cités introuvables** (`CONTRAT_PALMARES_*.md` vs réels `palmares_chaleur.md`/`palmares_froid.md`) ; palmarès min haute : chemin exact | le path filter « chaud » référence un `.md` inexistant (chemin mort) |
| Éclairage + garage | 4 | automations/helpers éclairage | lexicale | 4 contrats cités sous noms `CONTRAT_*.md` **introuvables** (réels : `contrats/eclairage/*.md`) | citations périmées systématiques |
...
| Présence / alarme / zones | 8 | automations/sensors présence, alarme, zones, BSSID, mobile, babysitting, visite, simulation | lexicale | `check_presence` : 4 chemins exacts + `--selftest` en CI (exemplaire, anti-régression assumée) ; alarme : chapitres sans chemin ; zones : nom introuvable | alarme : 963 lignes de regex sur un domaine de sûreté ; présence : couvre uniquement des invariants « déjà vrais » |
| Système / transverse | 13 | recorder, résilience, HINIT, notifications, batteries, redondance, UPS, paramètres invalides, vacances, VMC, voiture, sommeil, arsenal_self, registre chantiers | lexicale (résilience : structurelle à registre ; registre_chantiers : résolution de liens) | HINIT/sommeil/résilience : ancrage exact ; **recorder : « Arsenal Recorder Contract » introuvable** (pas de `contrats/recorder*.md`) ; **diagnostic Netatmo : aucun document trouvable** | résilience : mode `report` + `STRICT_ON_NEW=1` (dette gelée, nouveaux écarts bloquants — modèle intéressant) ; `check_vmc` : `ROOT=Path(".")` dépendant du cwd |

### 3.4 CI sans ancrage documentaire clair (normes fantômes ou citations mortes)

Constaté dans les scripts (en-têtes/docstrings) :

1. **Aucune citation, aucun docstring** : `check_aeration_m1`, `check_counters`, `check_deshum_guard`, `check_deshum_tx`, `check_deshumidificateur_metier`, `check_input_booleans`, `check_input_datetimes`, `check_input_texts`, `check_timers`.
2. **Contrat cité introuvable sous ce nom** (le document réel existe sous un autre chemin — citation périmée, pas norme fantôme) : `check_eclairage_entree/jardin/sejour`, `check_garage_toggle`, `check_volets_pluie`, `check_zones`, `check_palmares_temperature_journalier_chaud/froid`, `check_aeration_m2`.
3. **Aucun document normatif trouvable du tout** : `check_diagnostic_netatmo` (« Contrat v1.1 » sans trace), `check_recorder` (« Arsenal Recorder Contract » — seuls `architecture/01_recorder/` et des audits existent). Ces deux checkers vérifient des règles réelles mais **la norme qu'ils confrontent n'est écrite nulle part dans `contrats/`**.
4. **Règles de `doctrine.yml`** : ni l'interdiction `platform: template` ni l'obligation `mode:` ne citent de document (la seconde est plausiblement rattachable à `11_automations.md` / doctrine à écrire).
5. **Gates documentaires** (DOC-CI-*) : règles nommées et motivées dans les scripts, mais ancrées dans des **plans d'action datés** (`03_plans_action/transverses/`), pas dans une doctrine pérenne de `03_doctrines/`.
...
Neuf briques ont un checker dédié (01, 02, 03, 04, 05, 07, 08, 09, 19). **Sans checker structure** : `06_input_selects` (alors que `prefix_id.yaml` y est la source de vérité d'AID/APD), `10_scripts`, `11_automations` (couvert indirectement par AID/APD/doctrine.yml pour les ids et `mode:`, pas pour la structure), `12_template_sensors`, `13`–`17`, `18_lovelace` (couvert partiellement par les 3 checkers lovelace), `logbook`, `logger`, `recorder` (le checker recorder existe mais sans ancrage, cf. §3.4), `utility_meter`.
...
| Recorder | aucun `contrats/recorder*.md` | purge, populations A/B, dérogations | `contracts_recorder` (646 l.) | **CI sans ancrage documentaire clair** | la norme confrontée n'est écrite nulle part | P1 (écrire le contrat) |
| Diagnostic Netatmo | aucun document trouvable | fichier canonique, non-régression v1.1 | `contracts_diagnostic_netatmo` | **CI sans ancrage documentaire clair** | idem | P2 |
...
| **P1** | **Contrat recorder à écrire** (`contrats/recorder.md` ou promotion de `architecture/01_recorder/`), puis rattachement de `check_recorder` — correction documentaire préalable, pas nouvelle CI | norme aujourd'hui inexistante | norme fantôme de 646 lignes | recorder | rédaction | **Lot 1 (doc)** |
| **P1** | **Réparation des ancrages** : citations périmées (éclairage ×4, palmarès ×2, zones, aération M2, volets) et en-têtes absents (aération M1, deshum ×3, inputs ×5) — chaque checker cite son contrat par chemin exact (modèle AID/APD) | contrats existants, chemins réels | traçabilité D-NE-3 (« que vérifie cette règle ? ») | scripts checkers | faible (mécanique) | **Lot 1** |
...
| **P3** | Ancrage/contrat du diagnostic Netatmo | à écrire | norme fantôme mineure | météo | rédaction | backlog |
...
5. **Corrections documentaires préalables** : écrire le contrat recorder ; rafraîchir `REGISTRE_COUVERTURE_VERIFICATION.md` (compteurs 76/80, §5.2 périmé depuis AID/APD).
...
1. Écrire/promouvoir le **contrat recorder** (norme fantôme la plus volumineuse).
2. **Rafraîchir le registre de couverture** (dérive constatée §3.6) — idéalement en même temps que sa CI anti-dérive (lot 2), pour ne pas re-dériver.
3. Promouvoir les **invariants documentaires** (R-DOC-*, DOC-CI-*) en doctrine pérenne.
4. Statuer sur le **statut du document Fable** (promotion en doctrine arbitrée, maintien en évolution future, ou archivage motivé) — l'ambiguïté actuelle (« opposable » auto-déclaré dans `evolutions_futures/`) est elle-même un écart de gouvernance.
5. Ancrer ou documenter le **diagnostic Netatmo**.
