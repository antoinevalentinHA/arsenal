...
| **P1** | ⚠️ **PARTIEL — cette ligne couvre DEUX CI distinctes, une seule est faite.** **CI structure `06_input_selects` : reste OUVERTE** (aucun checker de structure sur cette brique au 2026-07-15 — vérifié). ~~contrôle dédié de `prefix_id.yaml` (format des options `NNNN - domaine`, unicité des préfixes ET des noms de domaine)~~ **✅ soldé (Lot 2, 2026-07-09)** : `check_prefix_id_contracts.py` + `contracts_prefix_id.yml`, bloquant. | `00_structure_includes/06_input_selects.md` (**ouvert**) + ~~`id_automatisations.md`~~ (**fait**) | corruption de la source de vérité des identités | `06_input_selects/` | faible | **Lot 2** |
