...
- **Statut :** ouvert — **Lot 1 (A–E) soldé, Lot 2 partiellement livré** ; restent 1C-c et le solde des lots 2–3. **Suivi canonique : ligne C14 du [`REGISTRE_CHANTIERS.md`](../../REGISTRE_CHANTIERS.md)** — pas ce document.
- **Base observée :** `main` @ `f583efe` (2026-07-04) — post passe contractuelle finale préfixes/domaines (#260)

> ### ⚠️ Ce document est un cadrage daté, pas un état des lieux courant
>
> **Les tables d'analyse (§3, §5, §6) décrivent la base `f583efe` du 2026-07-04 et n'ont pas été réécrites depuis.** Elles restent la trace du diagnostic initial. Plusieurs constats y sont **périmés** : le travail a été fait depuis.
>
> Pour ne pas rouvrir du travail soldé (**le biais agent que ce document décrit lui-même en §1.1**), chaque ligne concernée porte désormais un marqueur **✅ soldé** daté, renvoyant à son lot ou à sa PR. Une ligne **sans marqueur reste ouverte et exacte** ; une ligne **⚠️ partiel** n'est soldée qu'en partie — lire le marqueur avant d'agir.
>
> **Les marqueurs annotent, ils ne réécrivent pas** : le constat d'origine reste lisible, conformément au caractère d'instantané de ce cadrage (et au geste léger de la charte §91 — anti backlog déguisé).
>
> **En cas de doute, l'état réel fait foi via la ligne C14 du registre, jamais via les tables ci-dessous.** Et conformément au §1.4 : vérifier en ouvrant la cible, pas en lisant ce document.
>
> *(Réconciliation du 2026-07-15 — chaque marqueur a été établi en ouvrant le fichier cible sur `main`, pas en se fiant à un rapport.)*
...
|---|---|---|---|---|---|---|
| `validation.yml` | `yamllint` inline | tout le dépôt | lint YAML générique | aucun | **cosmétique** | ~~`\|\| true` : le job est toujours vert, jamais bloquant~~ — **✅ soldé (Lot 1C)** : le `\|\| true` est **retiré**. `validation.yml` exécute désormais `check_configuration_includes.py` en **bloquant** (résolution des includes de `configuration.yaml`), et `yamllint` reste **informatif assumé** (`continue-on-error`, plus de faux badge vert). **Reste ouvert (1C-c)** : aucune validation de **schéma HA** — un YAML valide-syntaxe mais invalide-HA passe toujours. |
| `doctrine.yml` | grep + Python inline | `12_template_sensors/`, `11_automations/` | interdiction `platform: template` legacy ; présence de `mode:` sur les automations | ~~aucun document cité~~ **✅ les deux règles citent leur source par chemin exact** (`12_template_sensors.md` §Invariants ; `11_automations.md` §Modes Arsenal) | ~~partielle~~ **bonne (sur son axe)** | **✅ soldé (Lot 1B)** — les trois limites sont levées : test `mode:` désormais **par automation** (parsing YAML, chaque item portant `id` doit porter son `mode`, robuste au multi-automations) ; placeholder mort `default_entity_id` **retiré** (arbitré : c'étaient 2 lignes de commentaire sans code — aucun contrôle supprimé, aucun introduit ; migration `default_entity_id` = sujet séparé non promis) ; grep sur `11_template_sensors` inexistant **retiré**. |
...
|---|---|---|---|---|---|---|
| Chargement HA | implicite (toute la norme le présuppose) | la configuration charge sans erreur | ~~`validation.yml` (non bloquant, syntaxe seule)~~ **⚠️ partiel depuis le Lot 1C** : `validation.yml` contrôle en **bloquant** la résolution des includes (`check_configuration_includes.py`) ; yamllint informatif assumé | ~~**non couvert automatisable**~~ **partiel** | **Écart restant (1C-c)** : aucune validation de schéma HA — un YAML valide-syntaxe mais invalide-HA passe toujours. Les **includes morts**, eux, sont désormais bloqués. | **P0** |
...
| Préfixes — source de vérité | `id_automatisations.md` §Création de domaine | structure/unicité des options de `prefix_id.yaml` | ~~consommée par AID/APD, jamais contrôlée pour elle-même~~ **✅ `contracts_prefix_id`** (`check_prefix_id_contracts.py`, bloquant — Lot 2, livré 2026-07-09) | ~~**partiel**~~ **couvert** | ~~une option malformée dégrade silencieusement AID-004/APD~~ — format « NNNN - domaine » et unicité préfixe/domaine désormais contrôlés pour eux-mêmes | ~~P1~~ **—** |
...
|---|---|---|---|---|---|---|
| **P0** | ⚠️ **PARTIEL.** **CI de chargement HA** (validation de configuration Home Assistant en conteneur, bloquante) — **reste ouvert, c'est le Lot 1C-c**. ~~et, en attendant, rendre `yamllint` bloquant ou retirer `validation.yml` (un check toujours vert est pire qu'absent)~~ **✅ soldé (Lot 1C)** : le faux badge vert est retiré, un contrôle **bloquant** de résolution des includes le remplace, yamllint est informatif assumé. **Attention** : rendre yamllint bloquant n'est **pas** un quick win — ~776 constats (302 errors / 474 warnings) à résorber d'abord (cf. §8.2). | présupposé de toute la norme ; à ancrer dans une doctrine courte | config qui ne charge pas / automation silencieusement désactivée | tout le YAML runtime | moyenne (image HA en CI) ; ~~quick win partiel : yamllint bloquant~~ | **Lot 1** |
...
| **P0** | ✅ **SOLDÉ (Lot 1B, 2026-07).** ~~**Durcissement `doctrine.yml`** : check `mode:` par automation (parsing YAML, pas par fichier) ; réactiver ou statuer le check `default_entity_id` ; supprimer le grep sur dossier inexistant ; ancrer les deux règles dans un document~~ — les quatre points sont faits : parsing YAML par automation ; `default_entity_id` **statué → retiré** (placeholder mort, aucun contrôle supprimé) ; grep mort retiré ; les deux règles citent leur source par chemin exact. | ~~à écrire/rattacher~~ **`12_template_sensors.md` §Invariants + `11_automations.md` §Modes Arsenal** (les deux existaient — cf. §1.4) | automation multi-fichier sans `mode:` = comportement runtime non maîtrisé | `11_automations/` | faible | **Lot 1** |
| **P1** | ✅ **SOLDÉ (Lot 1E).** ~~**CI frontière git / sécurité publication** : aucun artefact des catégories interdites (`secrets.yaml`, `.storage/`, `*.db`, certificats…) n'est tracké ; `.gitignore` couvre les catégories doctrinales~~ — `scripts/security/audit_publication_git.py` + workflow `security_publication_audit.yml`, **bloquant** depuis l'**incident P0 du 2026-07-05** (secrets Zigbee2MQTT publiés, #282) ; correctif encodage `--history` (1E-f, 2026-07-09). Le risque « fuite de secrets » n'était pas théorique : il s'est réalisé. | `git.md` + `contrats/publication/securite_publication_git.md` | fuite de secrets, pollution runtime du patrimoine | dépôt entier | faible | **Lot 1** |
...
| **P1** | ✅ **SOLDÉ — cette ligne couvrait DEUX CI distinctes, les deux sont faites.** **CI structure `06_input_selects` : ✅ soldé (2026-07-19)** — `check_06_input_selects_contracts.py` + `contracts_06_input_selects.yml` (bloquant), validation structurelle du patron name/options (ISS-000..007), 18/18 helpers conformes. ~~contrôle dédié de `prefix_id.yaml` (format des options `NNNN - domaine`, unicité des préfixes ET des noms de domaine)~~ **✅ soldé (Lot 2, 2026-07-09)** : `check_prefix_id_contracts.py` + `contracts_prefix_id.yml`, bloquant. | ~~`00_structure_includes/06_input_selects.md`~~ (**fait**) + ~~`id_automatisations.md`~~ (**fait**) | corruption de la source de vérité des identités | `06_input_selects/` | faible | **Lot 2** |
...
| **P2** | ✅ **SOLDÉ (Lot 1A).** ~~**Réparation des ancrages (toutes catégories §3.4)** : faire citer à chaque checker son contrat par **chemin exact** (modèle AID/APD). Couvre les citations par titre/version (recorder, netatmo, ~25 domaines), les noms littéraux périmés (éclairage ×4, palmarès ×2, zones, volets, aération M2), et les en-têtes absents (aération M1/M3/M4/M6, deshum ×3, inputs ×5, counters, timers, vacances, visite, vmc, structure 01/02/03/19).~~ — **62 checkers ré-ancrés par chemin exact, 14 déjà conformes.** *Note de vérification (2026-07-15) : un `grep` restreint à `contrats/`+`architecture/` fait apparaître ~9 checkers « non ancrés » — c'est un **faux trou** (§1.4) : ils citent `audits/`, `ui/` ou `configuration.yaml`, foyers normatifs légitimes hors `contrats/`.* Aucun contrat à écrire — tous existent | contrats existants, chemins réels | traçabilité D-NE-3 (« que vérifie cette règle ? ») ; supprime le risque qu'un agent croie la norme absente | scripts checkers | faible (mécanique) | **Lot 1** |
...
| **P2** | ✅ **SOLDÉ (Lot 1D).** ~~**CI anti-dérive du registre de couverture** : recompte des compteurs §3 du registre (mêmes commandes que le document) et échec si écart — transforme la discipline de co-commit en règle confrontée~~ — `check_ci_coverage_registry.py` + `contracts_ci_coverage_registry.yml`, **bloquant** : la dérive des compteurs est redevenue une ERROR CI. | `REGISTRE_COUVERTURE_VERIFICATION.md` §6 | la mesure de couverture ment (déjà 2 dérives) | registre + scripts + workflows | faible | **Lot 2** |
...
| **P3** | ✅ **SOLDÉ (#348 / #349 / #350, 2026-07-13).** ~~Hygiène workflows : renommer `clim_ventilation_contracts.yml` → `contracts_climatisation_ventilation.yml`, corriger le chemin mort du path filter palmarès chaud et l'auto-référence de `contracts_19_button_card_templates.yml`, fixer `ROOT` de `check_vmc`~~ — les trois faits : renommage (#349, dernière exception de nommage à suffixe supprimée), chemins morts (#348), `ROOT` ancré sur le script (#350, invocation cwd-indépendante). | conventions constatées au registre de couverture | confort/cohérence | workflows | triviale | **Lot 2** |
...
> **✅ Lot 1 soldé (A–E) — cette liste est une trace, pas une file d'attente.** Seul le point 1 reste partiellement ouvert (1C-c). Ne pas rouvrir les points 2 à 5.

1. ⚠️ **PARTIEL** — **Chargement HA** : ~~statuer sur `validation.yml` (bloquant ou suppression)~~ **✅ statué (Lot 1C)** : faux badge vert retiré, contrôle d'includes bloquant, yamllint informatif assumé. **Reste (1C-c)** : cadrer une CI de validation de configuration HA en conteneur — toujours le trou le plus en amont (P0).
2. ✅ **SOLDÉ (Lot 1B)** — ~~**Durcir `doctrine.yml`** (check `mode:` par automation, statut du check désactivé, ancrage documentaire des deux règles).~~
3. ✅ **SOLDÉ (Lot 1A)** — ~~**Réparer les ancrages des checkers existants** (citations périmées, en-têtes absents)~~ : 62 ré-ancrés, 14 déjà conformes. La contre-mesure au biais agent est en place — un agent qui ouvre un checker tombe sur le chemin exact de sa norme.
4. ✅ **SOLDÉ (Lot 1E)** — ~~**CI frontière git / sécurité publication** (P1, faible difficulté, risque élevé).~~ Le risque s'est réalisé avant la pose du garde-fou (incident P0 du 2026-07-05, #282) ; le scanner est bloquant depuis.
5. ✅ **SOLDÉ (Lot 1D)** — ~~**Corrections documentaires préalables** : rafraîchir `REGISTRE_COUVERTURE_VERIFICATION.md` (compteurs 76/80, §5.2 périmé depuis AID/APD).~~ Compteurs rafraîchis **et mis sous garde mécanique** (`check_ci_coverage_registry.py`, bloquant) : §3 du registre = **82 checkers / 87 workflows / 81 `contracts_*`** au 2026-07-15. *(Le contrat recorder n'est pas à écrire : il existe — cf. §3.4. Reste seulement à statuer sur son placement, item P3 — cf. §8.4.)*
