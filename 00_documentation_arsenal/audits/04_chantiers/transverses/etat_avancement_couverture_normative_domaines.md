...
3. **P3 — consolidations :** `deshumidificateur/guard.md §12`. *(`sante/cardio_nuit.md` — statut + renvois + nav — fait ; `couleurs` / `boutons` / `statistiques` — faux trou, fait : restent UI/architecture, aucun contrat ; `12_capteurs_observabilite_pure.md` — fait : stub orphelin retiré, observabilité couverte par 07→11 + `sensor:diagnostic`.)*
