...
| P3 ✅ | `chauffage/15_capteurs/12_capteurs_observabilite_pure.md` | **stub orphelin redondant** : absent de l'index 01→11, aucun lien entrant ; observabilité déjà couverte par `07_capteurs_diagnostics_structurants.md` + frontières 08→11 + étiquette `sensor:diagnostic` (`architecture/02_etiquettes/capteurs.md`) | **traité — fichier retiré** ; aucun document de remplacement (le compléter aurait créé une source concurrente) |
| P3 ✅ | `deshumidificateur/guard.md` (§12) | **faux trou documentaire** : les 7 helpers `input_text.deshum_guard_*` sont déjà créés (`04_input_texts/deshumidificateur/guard.yaml`), câblés (`10_scripts/deshumidificateur/guard_deshumidificateur.yaml`) et gardés CI (`check_deshum_guard_contracts.py`) ; libellé « à créer » corrigé en « implémentés » | **traité — aucun runtime requis** (correction de libellé périmé uniquement) |
...
3. **P3 — consolidations : fait.** *(`sante/cardio_nuit.md` — statut + renvois + nav — fait ; `couleurs` / `boutons` / `statistiques` — faux trou, fait : restent UI/architecture, aucun contrat ; `12_capteurs_observabilite_pure.md` — fait : stub orphelin retiré, observabilité couverte par 07→11 + `sensor:diagnostic` ; `deshumidificateur/guard.md §12` — fait : faux trou, helpers déjà implémentés/câblés/CI-gated, libellé corrigé.)* **Plus aucun P3 ouvert.**
