...
- **Statut :** point d’étape consolidé — **2 points P1 traités**, **4 P2 traités** (`sante/sommeil.md`, `meteo/fallback.md`, `meteo/extrema_jour_courant.md`, `electromenager`), **1 divergence fermée** ; chantier **toujours ouvert** (P2–P4 restants).
...
2. **P2 — domaines à documenter :** `poele` (contrat souverain **ou** renvoi explicite acté côté chauffage). *(`electromenager` documenté + indexé — fait.)*
...
> Les deux P1 de l’audit (`05_capteurs_parametrage_canonique.md` vide, `reveils`/`babyphone` non documenté) sont traités ; P2 traités : `sante/sommeil.md` (v1.0), `meteo/fallback.md` (v1.0), `meteo/extrema_jour_courant.md` (v1.0) et `electromenager` (contrat plat racine). L’ordre restant ne comporte plus de P1 ; il reste un seul P2 « domaine à documenter » : `poele`.
