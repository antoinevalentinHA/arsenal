...
| **Restitution durcie** | **Livrée (2026-07-21, post-clôture) — INV-CMD-8.** Correctif du défaut « État commande figé sur Confirmée après arrêt clim » : vérité unique `binary_sensor.audi_climatisation_active` (+ attribut `etat`, transit Couche 1) ; automation `10150000000006` (`restitution_climatisation_active.yaml`) = **notification persistante** active/dismiss **+** réconciliation `Confirmée → Au repos` au retour inactif, re-projection boot. Matérialisation d'état, aucune décision. **+ Backstop de fraîcheur** `10150000000007` (`rafraichissement_climatisation_active.yaml`, `refresh_vehicle_data` toutes les 5 min **tant que la clim est active**) : détecte promptement la **fin naturelle** de la clim ; sans lui, `climatisation_state` reste **périmé sur `cooling`** jusqu'au prochain poll → faux « actif » au dashboard + double notification (clim + charge revenue). |
...
| **Prochain jalon** | **E6** (échec véhicule non branché) via `script.audi_climatisation_observation` → consigner au §6, puis **clôture définitive** : lever la réserve, **retirer l'outil d'observation** temporaire, sortir C25 des Actifs. Essais résiduels E1b / E2–E5 opportunistes. |
