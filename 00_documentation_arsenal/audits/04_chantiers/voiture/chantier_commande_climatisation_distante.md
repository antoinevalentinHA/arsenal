...
| **Statut** | ✅ **CLÔTURE CONDITIONNELLE ACQUISE (2026-07-21).** Feuille de route déroulée dans l'ordre (doc → cadrage/contrat **A1** → runtime → UI → clôture). Faisabilité démontrée (reprise E1 : `climatisation_state=cooling` HA + myAudi « Actif » + corroboration physique) ; **nominal validé end-to-end** le même jour via la surface Arsenal (bouton → `input_select.audi_climatisation_commande_etat = Confirmée`, sur transition réelle observée). **Réserve unique : E6** (comportement en échec, véhicule non branché) à consigner pour lever la condition et retirer l'outil d'observation temporaire. Cf. [clôture conditionnelle](../../05_clotures/voiture/cloture_c25_commande_climatisation_distante.md). |
...
| **UI** | **Livrée puis simplifiée (2026-07-21), validation terrain en attente.** Section « ❄️ Commande climatisation » sur le dashboard normatif `18_lovelace/dashboards/voiture/audi.yaml`, **compacte** (arbitrage propriétaire : le dashboard principal n'est pas un dashboard de réglages) : **bouton d'action avec confirmation** (`carte_action_standard` → `script.audi_climatisation_distante`, **défauts 21 °C / comfort**) + **restitution lecture seule** `input_select.audi_climatisation_commande_etat` (`audi_etat_info`). Température/mode **non exposés** (défauts du script ; réexposition différée en dashboard Réglages si le besoin se confirme) ; helpers `audi_climatisation_temp_cible`/`audi_climatisation_mode_cible` **retirés**. Aucune logique UI (INV-CMD-7), aucun nouveau template, checkers UI verts. |
| **Restitution durcie** | **Livrée (2026-07-21, post-clôture) — INV-CMD-8.** Deux défauts terrain corrigés : (1) « État commande figé sur Confirmée » ; (2) « clim marquée active après sa fin naturelle + double notif ». Vérité **auto-expirante** : `binary_sensor.audi_climatisation_active_brut` (cloud, + attributs `etat`/`remaining`, transit Couche 1) → `binary_sensor.audi_climatisation_active` = brut **ET** avant l'échéance `input_datetime.audi_climatisation_fin_prevue` (= début + `remaining`, capturée par `10150000000007`, bornée [1,60] min déf. 30). Restitution `10150000000006` = **notification persistante** active/dismiss **+** réconciliation `Confirmée → Au repos`. **Cloud en simple confirmation** ⇒ robuste au retard **et** au **gel du climater** (403/404 → `support_climater=False`, early-return, jusqu'au reload — `audi_connect_account.py:900,964-970`). Le backstop « poller plus » (#501) a été **écarté** (inopérant si gelé). |
| **Outil d'observation** | **Temporaire (2026-07-21), à retirer à la clôture.** `script.audi_climatisation_observation` (`10_scripts/voiture/climatisation_observation.yaml`) : lance la commande puis journalise `climatisation_state` + `remaining_climatisation_time` toutes les 5 s pendant ~2 min (Logbook), pour remplir en un essai les 3 inconnues de mesure du §6. **Chaîne de détection vérifiée dans le code** : `climatisation_state` = valeur brute véhicule (endpoint `climater`/MBB, `audi_connect_account.py:906-908`) ; refresh post-action dans un **`finally`** (`:451-453`) avec `update_sleep` **5 s** par défaut (`const.py:28`) ⇒ témoin fiable succès **et** échec. |
| **Prochain jalon** | **(a) Diagnostic `last_changed`** de `sensor.audi_a3_sportback_e_tron_climatisation_state` — figé (⇒ **verrouillage climater** : sujet distinct de détection/reload à cadrer, casse aussi la commande) vs évolue seul (latence amont). **(b) E6** (échec véhicule non branché) via `script.audi_climatisation_observation` → §6, puis **clôture définitive** (lever la réserve, retirer l'outil d'observation, sortir des Actifs). Essais E1b / E2–E5 opportunistes. |
