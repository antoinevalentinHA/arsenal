...
| **UI** | **Livrée (2026-07-21), validation terrain en attente.** Section « ❄️ Commande climatisation » ajoutée au dashboard normatif `18_lovelace/dashboards/voiture/audi.yaml` (affordance manuelle, **aucune logique métier** — INV-CMD-7) : réglages `input_number.audi_climatisation_temp_cible` (tile) + `input_select.audi_climatisation_mode_cible` (tile), **bouton d'action avec confirmation** (`carte_action_standard` → `script.audi_climatisation_distante` avec les réglages), et **restitution lecture seule** de `input_select.audi_climatisation_commande_etat` (`audi_etat_info`). Aucun nouveau template, aucune couleur écrite à la main. 5 checkers UI verts. |
| **Prochain jalon** | **Clôture documentaire** (étape 5), **après validation terrain end-to-end** (appel du bouton → `En cours` → `Confirmée`/`timeout`). En parallèle : essais résiduels E1b / E2–E5 / E6. |
