...
| **Prochain jalon** | **Clôture documentaire** (étape 5), **après validation terrain end-to-end** : appel du bouton → `En cours` → `Confirmée`/`timeout`, et **E6** (échec véhicule non branché) via l'outil d'observation pour confirmer que `climatisation_state` reste inchangé (discriminant). En parallèle : essais résiduels E1b / E2–E5. |
