...
| **CH-4** | `ALM-MIN-2`, `ALM-IMP-3` *(requalifié Mineur post-V4)* | **Clôture conditionnelle acquise** — réserve : validation terrain `ALM-MIN-2` | `5892d35`, `476116e` |
...
- **CH-4** : lot **A** (`ALM-MIN-2`) — émetteur de bip unique, restreint aux origines explicites, hors mode test (`5892d35`) ; lot **B** (`ALM-IMP-3`) — automatisation morte `stop.yaml` supprimée, chemin d'arrêt réel inchangé (`arret_sirene` + durée device) (`476116e`). **Implémenté + déployé** ; **réserve unique** : validation terrain de `ALM-MIN-2`.
...
- **CH-4** — **clôture conditionnelle acquise**. **Lot CH-4-A (`ALM-MIN-2`)** : implémenté + déployé (`5892d35`) ; **validation terrain opportuniste en attente**. **Lot CH-4-B (`ALM-IMP-3`)** : implémenté + déployé (`476116e`) — automatisation morte supprimée, **résidu runtime résolu** (reste doc canonique → CH-5). **Réserve unique** : validation terrain de `ALM-MIN-2`.
...
3. 🟡 **Mineur** — MIN-1 (CH-3), MIN-2 (CH-4-A implémenté/déployé, validation en attente), MIN-3 / MIN-6 (CH-5), MIN-5 (CH-1, terrain) ; **`ALM-IMP-3` requalifié post-V4, résidu runtime résolu en CH-4-B (`476116e`)**.
...
Sur 6 chantiers : **CH-2 et CH-6 soldés**, **CH-1 et CH-4 en clôture conditionnelle acquise** (réserves : test positif `S3` ; validation terrain `ALM-MIN-2`), **CH-3 / CH-5 ouverts**. Le domaine **n'est pas clôturé**. La prochaine action à plus forte valeur est le **test positif `S3`** ; **CH-4 a ses deux lots implémentés + déployés** (`5892d35` MIN-2 ; `476116e` IMP-3 — automatisation morte supprimée), sa **réserve unique** étant la **validation terrain de `ALM-MIN-2`**.
