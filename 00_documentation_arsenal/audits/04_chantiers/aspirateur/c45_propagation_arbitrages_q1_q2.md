...
### 5.3 Lot 3 — Extension CI — **EXÉCUTÉ (2026-09-03)**
...
> **État : exécuté.** Trois contrôles neufs — **`ASP-CI-43`**, **`ASP-CI-44`**, **`ASP-CI-45`** —
> et **une extension sans nouveau numéro** d'`ASP-CI-11`. Le checker est **vert**, sa batterie
> d'auto-test passe de **670 à 702 cas**, et **aucun contrôle existant n'est affaibli** :
> `ASP-CI-16` et `ASP-CI-37` restent rouges sur les amputations qu'ils gardaient déjà. **Aucun
> runtime, aucun Lovelace, aucun contrat, aucun changelog.** Les items **3.4** et **3.6** sont
> **reportés au Lot 4** et l'item **3.1** est **lu comme livrant le mécanisme, non le nom** —
> voir la table des six décisions ci-dessous.

#### Décisions humaines rendues — préalables au lot (2026-09-03)

**Six décisions ont été rendues par l'opérateur propriétaire.** Elles ne sont **pas** de même
nature, et ce document ne les confond pas : **quatre** tranchent un **rang d'item** — un
séquençage —, **une** attribue des **identifiants de contrôle**, et **une** attribue un **chemin
de fichier** pour un lot à venir. Aucune ne rouvre `Q1` ni `Q2`. **Aucun code de référence ne leur
est attribué** : elles sont désignées par leur objet.

| Objet | Décision rendue | Nature |
|---|---|---|
| **Rang de l'inscription du lecteur pur** — item 3.1 contre item 4.3 | **Le Lot 3 livre le MÉCANISME ; le Lot 4 livre le NOM.** Voir l'encadré ci-dessous | **Décision de séquençage ratifiée** |
| **Items 3.4 et 3.6** — source exclusive et indisponibilité | **Reportés au Lot 4**, livrés dans le commit même de la projection : leur cible n'existe pas avant lui | **Décision de séquençage ratifiée** |
| **Mutation « rabattement sur `false` »** de la ligne *Preuve* | **Déplacée au Lot 4**, pour le même motif | **Décision de séquençage ratifiée** |
| **Item 3.8** — offre des gestes | **Scindé** : `3.8a` backend au Lot 3, `3.8b` autorité des quatre sites au Lot 6, **sans nouveau numéro** | **Décision de séquençage ratifiée** |
| **Numéros des contrôles neufs** | **`ASP-CI-43`**, **`ASP-CI-44`**, **`ASP-CI-45`** — vérifiés **libres** avant emploi, numérotation **continue** de 1 à 45 | **Attribution humaine d'identifiants** |
| **Chemin de la projection métier** | **`12_template_sensors/aspirateur/mission_arsenal_ouverte.yaml`** — **chemin attribué pour le Lot 4** ; au Lot 3 le **fichier n'est pas créé**, le **chemin n'est pas inscrit dans `LECTEURS_VERDICT`**, et **aucune autorisation dormante n'est créée** | **Attribution humaine de chemin pour le Lot 4** |

> **⚠️ Rang de l'inscription du lecteur pur — contradiction interne du présent chantier,
> tranchée.** L'item **3.1** plaçait au
> **Lot 3** l'ajout de la projection à l'exception nominative ; l'item **4.3** plaçait au **Lot 4**
> l'inscription du fichier à cette même exception ; et la **règle d'ordre n° 2** du §9 justifie
> l'antériorité du Lot 3 par le refus d'une *« autorisation dormante »*. **Les trois ne pouvaient
> être vrais ensemble.**
>
> **Fait établi par exécution.** Inscrire au Lot 3 le nom d'un fichier qui n'existera qu'au Lot 4,
> c'est **créer précisément l'autorisation dormante** que la règle n° 2 proscrit. Le dépôt refuse
> déjà ce motif ailleurs — `ASP-CI-31` refuse une allowlist nommant un fichier inexistant.
> Aggravant, et vérifié : la boucle « lit le verdict, ne l'écrit jamais » chargeait le fichier par
> `yaml.safe_load(… or "")` et **passait en silence** sur un fichier absent — la preuve portait
> alors sur un **document vide**, donc sur rien.
>
> **Décision rendue : le Lot 3 livre le contrôle de l'exception nominative** — nominativité,
> **existence**, frontière hors-Lovelace, lecture pure —, **et le Lot 4 inscrit le nom**, dans le
> **même commit** que le fichier. **L'item 4.3 prime sur l'item 3.1**, qui se lit en conséquence.
> Le mécanisme livré est **actif et vert** sur les deux lecteurs purs existants, et **rouge** sur
> une inscription anticipée : il n'est donc **pas dormant**.

| Item | Contenu |
|---|---|
| **3.1** | **Rang tranché : ce lot livre le mécanisme, le Lot 4 le nom.** Ce lot livre le **mécanisme** de l'exception nominative de lecture d'`ASP-CI-11`, **jamais le nom** : nominativité, **existence du fichier nommé**, **frontière hors-Lovelace**, et propriété « lit le verdict, ne l'écrit jamais ». **✅ Fait** — `refus_allowlist_lecteurs()`, vert sur les deux lecteurs purs existants, **rouge** sur une inscription anticipée et **rouge** sur un chemin Lovelace. L'inscription de la projection appartient au **Lot 4, item 4.3**. |
| **3.2** | **Nominativité préservée** — refuser toute autorisation par motif, famille ou répertoire. **✅ Fait** — `LECTEURS_VERDICT` reste un ensemble de **chemins littéraux** ; la frontière ajoutée refuse en outre tout chemin situé sous `18_lovelace/` ou `19_button_card_templates/`. |
| **3.3** | **Aucune lecture directe par Lovelace** : l'interdiction faite aux arbres Lovelace de mentionner le helper doit rester **mécaniquement** garantie **après** l'ajout de l'exception. C'est une **non-régression**, à prouver dans les deux sens. **✅ Fait, et prouvé dans les deux sens** — ① un arbre Lovelace qui mentionne le helper est **rouge** (`ASP-CI-11`, déjà en place, mutation rejouée) ; ② un chemin Lovelace inscrit à l'allowlist est **rouge** (extension neuve). Le second sens était la brèche : sans lui, le Lot 4 aurait pu relâcher l'interdit en nommant un lecteur au mauvais endroit (condition d'arrêt `A6`). |
| **3.4** | **Source exclusive** : prouver que la projection ne fait intervenir **aucun** témoin natif — ni état machine, ni témoin de session, ni entité `vacuum`. **➡️ REPORTÉ AU LOT 4**, par décision de séquençage : la projection n'existe pas, et un contrôle vert **par absence de cible** est **dormant**. Il est livré dans le commit même qui crée le fichier. |
| **3.5** | **✅ Fait — `ASP-CI-43`.** **Égalité exacte des représentations de classe** — désormais opposable par **`ASP-INV-98`** ([`15`](../../../contrats/aspirateur/15_conduite_et_supervision.md) §2). Étendre le patron `ASP-CI-37` aux **QUATRE représentations non gardées** du §3.2 rectifié : `verdict_ouvert` du script de conduite · `verdict_ouvert` de la supervision · **`engagements`** de la supervision, sous-ensemble nommé de la classe `O` · **clés de `phrases`** de la projection, énumération exacte de la classe `O`. Le recensement s'établit **par lecture des variables déclarées** dans le périmètre nominatif du checker, **jamais** par recherche d'une clé littérale — c'est précisément ce qui avait manqué les deux dernières. |
| **3.6** | **Indisponibilité non rabattue** : refuser une projection faisant valoir `false` à `unknown`, `unavailable` ou à une valeur hors vocabulaire. **➡️ REPORTÉ AU LOT 4**, par la même décision de séquençage et pour le même motif que l'item 3.4. |
| **3.7** | **Reliquats de l'ancien nom — DEUX RÉGIMES MÉCANIQUES SUCCESSIFS.** Le contrôle ne peut pas exiger zéro occurrence dès le Lot 3 : la décision `H-3` **autorise** l'ancien code jusqu'au mouvement atomique du Lot 5. Il doit néanmoins être **immédiatement actif et utile**, sans prétendre que la migration a déjà eu lieu.<br><br>**① Régime transitoire — livré au Lot 3, actif dès le Lot 3.** Contrôle **actif** des occurrences de l'ancien nom, adossé à une **allowlist transitoire fermée et nominative** énumérant les occurrences **encore autorisées** — celles qui existent au HEAD du Lot 2, et elles seules. **Rouge** : toute occurrence **supplémentaire**, toute occurrence **hors allowlist**, et **toute apparition prématurée du nouveau code** avant le Lot 5. Ce régime **ne crée aucun alias** et **aucune double exposition runtime** : il n'autorise pas un second nom, il **gèle** la population du premier.<br><br>**② Régime permanent — activé au Lot 5, dans le mouvement atomique.** **Suppression** de l'allowlist transitoire, **activation** de la règle permanente exigeant **zéro occurrence** de l'ancien nom dans **tout le périmètre gouverné**, et **suppression obligatoire** de la clause transitoire du chapitre [`08`](../../../contrats/aspirateur/08_etats_et_observation.md) §1.3. La bascule entre les deux régimes est **livrée dans le même commit** que la migration (§5.5).<br><br>**✅ Fait — `ASP-CI-44`.** L'allowlist gèle **quatorze emplois techniques** : le producteur (1 clé d'attribut), les quatre sites Lovelace (4 slots de restitution), ce module (6) et le chapitre `08` (3).<br><br>**Ce que le contrôle compte, et ce qu'il ne compte pas.** Chez le producteur et dans l'arbre Lovelace, le cardinal vaut **deux fois** : en **forme**, l'analyse structurée compte les clés d'attribut et les slots de restitution, et elle seule fait autorité sur ce que ces emplois **sont** ; en **nombre**, le même cardinal borne le **total** des occurrences du code dans le fichier, **commentaires neutralisés**. C'est cette seconde lecture qui rattrape un emploi que la forme ne voit pas — un `state_attr(…, '‹code›')` en Jinja, une carte markdown qui restitue l'attribut. La neutralisation des commentaires est **tout ce qui sépare cette règle d'un recomptage brut** : une **mention** ne répand rien et reste **verte**, un **emploi** est rouge. Partout ailleurs dans le YAML gouverné, le token est refusé sans exception.<br><br>**Contrats : les cibles de liens sont neutralisées d'abord.** Le fichier d'arbitrage `Q1` porte le code de remplacement dans son **nom** ; un chapitre doit pouvoir renvoyer à la note qui fonde la règle. Seule la **cible** du lien est neutralisée — son **libellé** et le reste de la ligne restent lus, de sorte qu'un emploi technique glissé dans le texte d'un lien reste refusé.<br><br>**Portée contractuelle.** Le chapitre `08` est le **seul** où le token historique reste admis, à cardinal exact et tant que vit sa clause §1.3 ; **tout autre chapitre du contrat Aspirateur qui l'emploierait est rouge**, de même que toute apparition prématurée du code de remplacement. La **formulation métier** — « mission Arsenal ouverte », « session robot active », avec espaces — reste **libre partout** : c'est elle que le Lot 2 a inscrite. Documents historiques, arbitrages, audits, registres et le présent chantier restent **hors** de cette interdiction technique.<br><br>**Pourquoi la détection ne peut pas être textuelle sur tout le dépôt.** Le code de remplacement figure **déjà dix-sept fois** — dans le nom du fichier d'arbitrage `Q1`, dans les liens qui le citent, et dans les mentions d'attribution de ce chantier et du registre. Une recherche de chaîne brute aurait été **rouge dès sa livraison, sur l'arbitrage même qui la fonde**. **Preuve du Lot 3** : mutation rouge sur chacun des trois refus du régime ① — une occurrence ajoutée, une occurrence déplacée hors allowlist, une apparition anticipée du nouveau code. **Preuve du Lot 5** : recherche d'absence à **zéro occurrence**, allowlist transitoire **absente du checker**, clause `08` §1.3 **absente du contrat**. |
| **3.8a** | **✅ Fait — `ASP-CI-45`.** **Offre des gestes**, moitié **backend et non-régression** : la garde de classe `O` **précède** le dispatch et n'écrit rien ; la branche `arret` ne porte **aucune** abstention de sens physique — c'est l'**indépendance d'Arrêt** à l'égard du témoin natif, rendue opposable ; la branche `retour_base` porte ses **trois** exclusions, ni plus ni moins ; et, côté interface, le site « Arrêter la mission » ne porte **aucune** exclusion d'état tandis que « Renvoyer à la base » porte exactement les siennes. Ces deux propriétés d'interface **survivent au changement d'autorité du Lot 6**, et c'est ce qui les rend gardables dès maintenant. |
| **3.8b** | **➡️ LOT 6**, par décision de séquençage — l'**autorité** des quatre sites Lovelace. Ils lisent aujourd'hui l'attribut dérivé du témoin natif, non la projection métier. L'affirmer ici serait **faux** ; le taire laisserait croire que le contrôle couvre plus qu'il ne couvre. `ASP-CI-45` sera **étendu**, **sans nouveau numéro**. |
...
| **4.3** | **Inscrire le fichier à l'exception nominative livrée au Lot 3, DANS LE MÊME COMMIT que sa création.** L'extension d'`ASP-CI-11` refuse une allowlist nommant un fichier inexistant : une inscription anticipée est **rouge**, une création sans inscription laisse le fichier hors périmètre. Les deux vont donc ensemble, et l'ordre entre elles n'existe pas.<br><br>**Trois inscriptions, et non une — au même commit que la création.** `12_template_sensors/aspirateur/mission_arsenal_ouverte.yaml` devra être ajouté à :<br>① **`LECTEURS_VERDICT`** — sans quoi `ASP-CI-11` refuserait le fichier, qui mentionne le helper de verdict ;<br>② **`PERIMETRE_REPRESENTATIONS`** — sans quoi toute représentation de classe qu'il embarquerait échapperait à `ASP-CI-43`, ce périmètre étant **nominatif** et non déduit ;<br>③ **`REPRESENTATIONS_ATTENDUES`** — avec son cardinal, sans quoi le recensement figé à sept refuserait toute représentation qu'il porterait, ou la laisserait non qualifiée.<br><br>**Aucune de ces trois inscriptions n'est faite au Lot 3**, et aucune ne peut l'être : la première serait une autorisation dormante, les deux autres nommeraient un fichier illisible. |
| **4.4** | **Source exclusive** — item 3.4, reporté ici par décision de séquençage : prouver que la projection ne fait intervenir **aucun** témoin natif — ni état machine, ni témoin de session, ni entité `vacuum`. |
| **4.5** | **Indisponibilité non rabattue** — item 3.6, reporté ici par la même décision : refuser une projection faisant valoir `false` à `unknown`, `unavailable` ou à une valeur hors vocabulaire. **Mutation rouge obligatoire** : le rabattement d'indisponibilité sur `false`. |
...
### 5.6 Lot 6 — Adaptation de l'interface — **EXÉCUTÉ (2026-09-07)**

**Dépend des Lots 4 et 5.**

> **Exécution.** Les **quatre sites** du §3.4 lisent désormais l'autorité correcte, et
> `RC-02` est **levée** : l'affichage et la garde du backend lisent **le même prédicat**.
>
> **La tuile « Mission » est devenue deux tuiles**, comme le §3.4 l'exigeait. **Mission** consomme
> la **projection métier** (`ASP-INV-96`) — l'interface ne lit jamais le verdict (`11` §2) — et rend
> son indisponibilité au gris atténué, qui ne vaut **pas** « aucune mission » (`ASP-INV-45`).
> **Session robot** rend l'attribut orthogonal, sous des **libellés distincts** (`Active` /
> `Inactive`, contre `Ouverte` / `Aucune`) et en **gris neutre** : elle n'engage aucune
> responsabilité Arsenal et ne conditionne plus aucun geste — lui prêter le bleu de la Mission ferait
> lire deux fois le même fait, ce que `Q1` proscrit précisément.
>
> **Le bloc d'entrée passe donc à 2 × 2** — décision opérateur du 2026-09-07 : ligne 1, les deux
> lectures de tête (ce que fait la machine, ce dont Arsenal répond) ; ligne 2, les deux observations
> qui les nuancent (session, charge). **La quatrième tuile n'est pas un remplissage** — et c'est ce
> qui la distingue de celle qu'un arbitrage avait retirée à `C50` : *Session robot* et *Mission* sont
> **deux notions** d'autorités exclusives et non substituables, dont la divergence est **légitime**.
> Les rendre séparément est une **obligation contractuelle**, pas un choix de mise en page.
>
> **Les trois gardes de conduite** — section, `Retour base`, `Arrêt` — quittent le témoin natif pour
> la projection. L'**Arrêt** ne porte **aucune** condition de sens physique ; le **Retour base**
> conserve ses **trois** exclusions **telles quelles**. La **navigation est inchangée** (§3.5), et
> **ni Pause ni Reprise** ne sont touchées (§8).
>
> **`ASP-CI-45` est étendu sans nouveau numéro.** Jusqu'ici il gardait la **forme** de l'offre — il
> ne pouvait rien dire de l'**autorité**, la projection n'existant pas. Il vérifie désormais que les
> deux sites de geste **et** la disjonction de section conditionnent sur la projection, et
> qu'**aucun** ne lit plus l'attribut natif. Les **deux fautes symétriques** sont jouées : l'offre
> qui quitte la projection, et celle qui retourne au témoin natif. La **section** est vérifiée à
> part, parce qu'un geste offert par sa condition propre mais **retiré par sa section** n'est pas
> offert du tout — et cela ne se voit dans aucun site pris isolément.
>
> **`ASP-CI-44` voit son cardinal de slots passer de quatre à un**, et c'est une **bascule
> d'autorité, pas une disparition** : les trois gardes ont changé de source, la restitution reste.
> Le contrôle l'a **détecté de lui-même** au moment de la bascule ; le cardinal a été **qualifié**,
> jamais ajusté après coup pour faire taire un rouge.
>
> **Aucun contrat amendé** : le `11` §2 et §3 portaient déjà cette autorité depuis le Lot 2 — ce lot
> l'**exécute**. Auto-test **728 → 732 cas**, aucun cas perdu. **Aucun changelog.**
...
### 5.7 Lot 7 — Vérification et clôture — **EXÉCUTÉ (2026-09-07) — CLÔTURE NON ACQUISE**

> **Ce lot établit un fait qui empêche la clôture, et c'est son résultat principal.** Les items 7.1 à
> 7.3 sont verts, le scénario **(b)** passe, et le scénario **(a)** passe **pour les deux gestes que
> l'arbitrage a couverts**. Il **échoue pour Pause et Reprise**, dont `Q2` §6 *in fine* a
> explicitement laissé les conditions **non arbitrées**. `RC-02` ne peut donc pas être fermée.

#### Ce qui est acquis

| Item | Résultat |
|---|---|
| **7.1** | **Vert.** Lint documentaire, `DOC-CI-2/3/5/6`, contrôle structurel du registre (`REG-1`, `REG-2`). |
| **7.2** | **Vert.** Checker Aspirateur, **48 contrôles**, **0 écart**. Batterie d'auto-tests **702 → 739 cas** depuis le Lot 3, **aucun contrôle existant affaibli** — les seules baisses ont été **nominatives**, chaque cas retiré partant **avec l'obligation qu'il défendait**. |
| **7.3** | **Vert, et les deux recherches sont distinctes.** **Ancien nom d'attribut** : **0** occurrence dans le YAML gouverné, **0** dans les chapitres du contrat, **1** dans le module de contrôle — sa **déclaration**, un contrôle devant nommer ce qu'il refuse. **Helper de verdict hors allowlist** : **0** occurrence, et **0** dans les arbres Lovelace. L'allowlist est passée de **neuf à dix** fichiers — le dixième est la **projection métier** créée au Lot 4, **par construction et non par dérive**. |
| **7.4 (b)** | **PASSE.** Mission Arsenal ouverte, robot rentré au dock : **Arrêt offert**, **Retour base masqué**, et le contrôle vérifie que la projection y vaut bien `on` — sans quoi l'exclusion viendrait d'une mission absente et non du **seul motif de sens physique**, et le scénario ne prouverait rien. |

#### `R-LOT7-1` — le résidu qui empêche la fermeture de `RC-02`

**Fait établi mécaniquement**, en rendant la chaîne entière — valeur native → gabarit d'état →
gabarit de projection → conditions réelles de l'arbre Lovelace → offre :

| Monde simulé | Arrêt | Retour base | Pause | Reprise |
|---|---|---|---|---|
| **(a)** mission **externe**, robot en nettoyage | masqué ✅ | masqué ✅ | **OFFERT ❌** | masqué |
| **(a bis)** mission **externe**, robot en pause | masqué ✅ | masqué ✅ | masqué | **OFFERT ❌** |
| **(b)** mission Arsenal, robot rentré au dock | **offert ✅** | masqué ✅ | masqué | masqué |
| **(d)** verdict **inconnaissable** au démarrage | masqué ✅ | masqué ✅ | **OFFERT ❌** | masqué |

**Pourquoi.** Le Lot 6 a fait basculer l'autorité de l'**Arrêt** et du **Retour base** sur la
projection métier — c'est ce que `Q2` avait arbitré. Les conditions de **Pause** et de **Reprise**
sont restées adossées à l'**activité physique** (`état canonique = nettoyage_reel`, `= pause`), parce
que `Q2` §6 *in fine* et §8 les ont **explicitement laissées hors arbitrage**.

**Or la garde du script de conduite s'arrête AVANT le dispatch**, sur le seul verdict
([`conduire_mission.yaml`](../../../../10_scripts/aspirateur/conduire_mission.yaml), étape 2 :
*« Aucune mission Arsenal ouverte : le verdict n'est pas de classe O »*). Un **Pause** émis pendant
une mission externe est donc **refusé**. C'est, mot pour mot, `RC-02` — **un bouton présenté puis
ignoré** —, subsistant sur les deux gestes que l'arbitrage n'a pas couverts.

**Ce n'est pas une régression du Lot 6**, et ce n'est pas non plus un oubli : c'est la **conséquence
mécanique d'un périmètre d'arbitrage**, restée invisible tant que personne n'avait rendu l'offre.

**Ce que le lot en fait.** `ASP-CI-48` **gèle** ce résidu — ses attendus disent **ce qui est**, pas
ce qui devrait être —, exactement comme l'allowlist du Lot 3 gelait une population. Le résidu ne peut
plus **s'étendre en silence**, et le jour où un arbitrage étendra l'autorité de la mission à Pause et
Reprise, **ces attendus deviendront rouges**, donc visibles. **Aucun correctif n'est appliqué** :
l'appliquer serait trancher, dans un lot d'exécution, une question que `Q2` a délibérément laissée
ouverte.

#### 7.5 — Scénarios terrain, et pourquoi ils sont peu nombreux

**Restreints à ce que la preuve statique ne peut pas atteindre**, et **disjoints de `C42`** — aucune
preuve partagée. La chaîne d'offre étant désormais **rendue mécaniquement**, il ne reste que ce qui
dépend de l'**appareil réel** ou de l'**instance** :
...
**Aucun de ces quatre scénarios ne recoupe ceux de `C42`**, qui portent sur la déclaration
d'entretien, ses compteurs et sa notification (§7).

#### 7.6 — Conditions de fermeture : deux constats fermables, un qui ne l'est pas

| Constat | Conditions | État |
|---|---|---|
| `AUD-ASP-01` · `CC-01` | (1) Lot 2 ratifié · (2) Lot 5 livré, **zéro occurrence** · (3) garde mécanique verte et **mutation rouge démontrée** · (4) aucun document normatif n'emploie plus le nom ambigu | **Les quatre sont réunies.** La condition (3) nomme l'item 3.7 du Lot 3 — l'allowlist transitoire —, que le Lot 5 a **supprimée en lui substituant** le régime permanent de zéro occurrence, vert et démontré rouge sur régression. **C'est une lecture** : la substance de la condition est satisfaite, sa lettre désigne un mécanisme qui n'existe plus. **Fermeture proposée, non prononcée.** |
| `RC-02` | (1) Lot 4 livré · (2) Lot 6 livré · (3) items 3.3 et 3.8 verts · (4) **scénarios 7.4 (a) et (b) passés** · (5) **aucun bouton présenté puis refusé** | **(1) (2) (3) réunies. (4) et (5) NE LE SONT PAS**, pour le seul motif de `R-LOT7-1`. **`RC-02` reste OUVERTE.** |

> **Ce que ce lot ne fait pas.** Il ne **prononce** aucune fermeture — l'établissement des conditions
> et leur qualification sont deux actes distincts, et le second appartient à l'opérateur propriétaire.
> Il ne clôt pas `C45` : le §5.7 exige les conditions du §6, et l'une d'elles n'est pas réunie.
> Il n'applique **aucun correctif** à `R-LOT7-1`.

#### La décision qui reste due

**Étendre l'autorité de la mission Arsenal à Pause et à Reprise ?** C'est un **arbitrage**, pas une
exécution : `Q2` §6 *in fine* les a laissées hors de son périmètre, et rien dans les documents
existants ne le tranche. Deux voies s'offrent, et elles ne sont pas équivalentes :

- **Aligner** — les quatre gestes suivent la mission Arsenal. `RC-02` se ferme, et le domaine cesse
  d'offrir le moindre geste sur une mission externe. **Conséquence assumée** : Arsenal n'offre plus
  *aucune* prise sur un robot qu'il n'a pas lancé, y compris pour le mettre en pause.
- **Contractualiser une capacité distincte** — un pilotage physique hors mission Arsenal, que `Q2`
  §6.4 nomme déjà comme **capacité distincte exigeant un contrat propre**. `RC-02` ne se fermerait
  alors qu'après ce contrat.

**Tant que cette question n'est pas tranchée, `RC-02` reste ouverte et `C45` ne peut pas être
clôturé.** Le résidu est **gelé, nommé et rendu visible** — c'est tout ce qu'un lot d'exécution peut
faire sans usurper un arbitrage.
...
2. **Le Lot 3 précède le Lot 4** : le **contrôle** existe avant l'objet qu'il garde. Mais il le
   **précède**, il ne le **devance pas** : le Lot 3 livre le **mécanisme** de
   l'exception nominative, le Lot 4 y inscrit le **nom**, dans le commit même qui crée le fichier.
   Inscrire le nom plus tôt serait exactement l'**autorisation dormante** que cette règle proscrit,
   et le contrôle livré au Lot 3 la refuse mécaniquement. Pour le même motif, un contrôle dont la
   cible n'existe pas avant le Lot 4 — source exclusive, indisponibilité — est **livré au Lot 4**,
   jamais rendu vert par l'absence de son objet.
