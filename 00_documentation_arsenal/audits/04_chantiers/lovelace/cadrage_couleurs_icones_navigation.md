...
> **Post-solde (2026-08-29) — Aspirateur, lots `U1` puis `U2` :** une **dix-neuvième** autorité de couleur NAV est posée, `sensor.etat_aspirateur_dashboard`, **avant** sa tuile (`U1`), puis la tuile entre dans la grille et NAS en sort (`U2`) — §2 decies. La tuile Aspirateur est la seule de la grille **sans destination** : le domaine n'a pas de dashboard dédié.
...
>
> **Complété au lot `U2`.** La tuile Aspirateur entre en ligne 4, quatrième place ; la tuile NAS sort de la grille ; Santé passe en tête de ligne 5. Vingt tuiles avant, vingt après. La tuile Aspirateur porte `tap_action: action: none` : **aucune destination**, la décision 1 du cadrage Aspirateur excluant tout dashboard dédié — l'interface du domaine est une **carte opérationnelle inline** de la même page. Le colorant NAS `sensor.etat_nas_dashboard` reste **intact** (arbitrage `A-7`) ; il n'a désormais **plus aucun consommateur**, et sa suppression relève du **lot de propreté séparé** que l'arbitrage prévoit. L'accès à `/nas-dashboard` est **déplacé** dans le dashboard Système, jamais supprimé.
