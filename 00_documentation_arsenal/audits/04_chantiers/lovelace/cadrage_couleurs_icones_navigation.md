...
> **Post-solde (2026-08-29) — Aspirateur, lots `U1`, `U2` puis `U3` :** une **dix-neuvième** autorité de couleur NAV est posée, `sensor.etat_aspirateur_dashboard`, **avant** sa tuile (`U1`) ; la tuile entre dans la grille et NAS en sort (`U2`) ; la tuile reçoit sa destination `/aspirateur-dashboard` et la carte opérationnelle quitte `navigation.yaml` (`U3`) — §2 decies. Les dix-neuf tuiles de la grille naviguent désormais toutes.
...
> **Statut :** **✅ implémenté (runtime, 2026-07-23)** — instancie l'**Option B** (formaliser une exception), restée ouverte depuis §2 (col. « B »). Étend l'**Exception 3** : le jaune ambré opaque `rgb(255, 193, 7)` entre dans la palette d'icône NAV/HUB. Artefacts : `ui/couleurs/03_exceptions.md` (Exception 3), `scripts/arsenal_contracts/check_ui_runtime_colors_contracts.py` (whitelist `ALLOWED_RGB`), `bouton_navigation_dynamique.yaml` (clé `vigilance`), `sensor.etat_arrosage_dashboard`.
...
**Ce que le jaune NAV est / n'est pas.** Teinte **ambre** opaque `rgb(255, 193, 7)` (et non le jaune canon `rgba(255, 235, 59, 0.2)`, trop clair pour tenir en aplat d'icône sur tuile claire — retour terrain « jaune sur gris illisible » ; l'ambre est la teinte « vigilance intermédiaire » déjà admise, Exception 6). Sens **unique et opposable** : « un signal existe, il est pris en charge — à savoir ». Non décisionnel, non bloquant, **strictement sous le rouge** (R3 : la vigilance ne masque jamais un rouge). L'orange `rgb(255, 152, 0)` reste hors palette NAV (il code un avertissement, pas une vigilance prise en charge).
...
## 2 decies. Aspirateur — *autorité de présentation posée avant la tuile (lot `U1`)*

> **Statut :** **✅ implémenté** — **Option C (dynamiser)**, instruite à partir des précédents du présent dossier. Artefacts : `sensor.etat_aspirateur_dashboard` (`12_template_sensors/system/cartes_dashboard_navigation/aspirateur.yaml`, lot `U1`) et la tuile qu'il colore dans `navigation.yaml` (lot `U2`).
>
> **Complété au lot `U2`.** La tuile Aspirateur entre en ligne 4, quatrième place ; la tuile NAS sort de la grille ; Santé passe en tête de ligne 5. Vingt tuiles avant, vingt après. Le colorant NAS `sensor.etat_nas_dashboard` reste **intact** (arbitrage `A-7`) ; il n'a désormais **plus aucun consommateur**, et sa suppression relève du **lot de propreté séparé** que l'arbitrage prévoit. L'accès à `/nas-dashboard` est **déplacé** dans le dashboard Système, jamais supprimé.
>
> **Corrigé au lot `U3`.** La première livraison de `U2` posait la carte opérationnelle **inline dans `navigation.yaml`** et laissait la tuile sans destination (`tap_action: none`) — lecture littérale de la décision 1 du cadrage Aspirateur. **Le résultat était mauvais** : trois cent quatre-vingts lignes de carte métier au milieu d'un menu de navigation, et une tuile qui ne réagissait pas au clic. **L'opérateur a tranché** : la décision 1 est **renversée**. Le domaine reçoit un dashboard dédié, `/aspirateur-dashboard`, la carte opérationnelle y est **déplacée sans être réécrite**, et la tuile navigue comme les dix-huit autres. `navigation.yaml` redevient ce qu'il doit être : une grille de tuiles, et rien d'autre.

**Pourquoi un capteur posé avant son consommateur.** Le cadrage Aspirateur impose la séquence `U1 ──► U2` : `U1` **n'enlève rien** et ne pose aucune tuile ; `U2` échangera NAS contre Aspirateur d'un seul geste. Poser l'autorité de couleur d'abord rend `U2` purement Lovelace, et laisse le lot `U1` **réversible** — supprimer ce fichier ne casse aucun rendu, puisque aucune carte ne le lit encore.

**L'état latent disponible.** Deux autorités backend, toutes deux déjà livrées : `sensor.aspirateur_etat_canonique` (lot `L1` — dix états, plus l'attribut `classe_partition` portant les quatre classes `R`/`A`/`E`/`N` du contrat `07` §5.0) et `binary_sensor.aspirateur_entretien_requis` (lot `M1` — dû / non dû, **indisponible** quand la question n'est pas tranchable). Le **verdict de mission** est hors d'atteinte : `ASP-CI-11` en réserve la mention à des fichiers nommés et balaie `12_template_sensors/` **commentaires compris**. « Mission ouverte » n'est donc pas la source du cycle — c'est l'état canonique, qui dérive des témoins de l'appareil.

**Mapping retenu — échelle de sévérité :**
...
**Ce que ce colorant ne fait pas.** Il ne se substitue pas à l'exposition distincte des dix états canoniques (`ASP-INV-44` interdit l'agrégation) : il s'y **superpose**, comme les dix-huit autres colorants NAV. Il ne reproduit aucune table de classification — il lit `classe_partition`, autorité unique de la partition. Aucun contrôle `ASP-CI` neuf n'est créé : le lot n'ouvre aucune règle nouvelle.
