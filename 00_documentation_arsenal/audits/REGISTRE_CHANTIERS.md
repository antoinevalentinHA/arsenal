...
| **C9** | Alarme — stabilisation atomique de l'absence sécurité | alarme | **P1** | phase 1 appliquée (v16.0.5, CI verte) — cause = race d'armement au front OFF de `binary_sensor.presence_famille_securite` ; décision = création de `binary_sensor.presence_famille_securite_absence_confirmee_alarme` (`delay_on 5 min`, atomique) ; ancien trio (timer + automation + `absent_depuis_5_min`) **conservé mais déclassé / diagnostic** | **Valider en runtime** post-déploiement (front OFF → armement +5 min, plus de ~10 ms), puis **phase 2 = suppression différée** du trio. Validations : `check_presence_contracts`, `check_alarme_contracts`, parse YAML, validation runtime post-déploiement | [`../changelog/changelogs/v16/v16_0_5.md`](../changelog/changelogs/v16/v16_0_5.md) · [`../contrats/alarme/30_decision_centrale.md`](../contrats/alarme/30_decision_centrale.md) |
| **C10** | Arrosage — V1 automatique & découvrabilité du domaine | arrosage | P2 | V1 runtime **mergée** (#125), puis **durcissements post-V1 mergés** (#126→#143) : UI, observabilité, fraîcheur, coexistence fail-safe et intention explicable. Alignements sur contrats existants, **sans nouvelle doctrine**. **Ensemble non publié** ; pas de changelog hors release. | Poursuivre le chantier arrosage via un **plan d'action vivant séparé** ; préparer la release **uniquement lorsque le domaine sera réellement livrable**. | [`../contrats/arrosage/README.md`](../contrats/arrosage/README.md) |
...
| **D-NAV-COULEUR** | Couleurs d'icônes des tuiles de navigation vs Exception 3 | lovelace / UI | **dossier d'arbitrage dormant** (cosmétique, aucun impact runtime) | Arrosage résorbé (→ dynamique, cf. C10) ; **reliquat** : ~7 tuiles du menu principal (dont **NAS `#1976D2` = bleu interdit par la charte**) + section ⚙️ Système, à couleur d'icône figée hors palette NAV. Réveil sur refonte charte couleurs / menu NAV, ou décision d'harmoniser. Arbitrage : **A** neutraliser · **B** formaliser une exception « identité NAV » · **C** hybride. | [`04_chantiers/lovelace/cadrage_couleurs_icones_navigation.md`](04_chantiers/lovelace/cadrage_couleurs_icones_navigation.md) |
