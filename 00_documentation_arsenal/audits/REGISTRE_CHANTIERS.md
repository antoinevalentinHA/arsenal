...
| **C6** | Libellés courts Android Auto (friendly_name) | ui / customize | P2 | **Lot 1 appliqué** via `01_customize` uniquement (commit `b4877674`) : 12 libellés courts — températures min/moy./max chambres-maison, 3 tendances, `clim_mode_local`, `ecs_temperature_ballon_securisee`, 4 scripts volets (le détail fait foi dans le runtime `01_customize/*.yaml`). Tendances et `clim_mode_local` en **`friendly_name` seul** : icônes dynamiques d'état **non écrasées**. **Validation visuelle Android Auto effectuée** le 2026-06-18 (terrain, écran Android Auto) : libellés lisibles et exploitables (volets, états principaux, températures et tendances) et grandes icônes de tendance validées. Hors-périmètre volontaire confirmé intact : `sensor.programme_chauffage`, `alarm_control_panel.alarme_maison`, `binary_sensor.ouvertures_maison`, `binary_sensor.mouvement_maison`. **Lot 2 en attente** (`programme_chauffage`, `alarme_maison`) — non ordonnancé : requiert le **verrouillage Lovelace préalable** des cartes dépendant implicitement du `friendly_name` (history-graph chauffage/alarme + `carte_alarme_decision`). | [`../architecture/00_structure_includes/01_customize.md`](../architecture/00_structure_includes/01_customize.md) § *friendly_name & surfaces d'affichage* |
