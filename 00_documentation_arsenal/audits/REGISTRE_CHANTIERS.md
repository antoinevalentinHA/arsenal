...
| **C6** | Libellés courts Android Auto (friendly_name) | ui / customize | P2 | **Lot 1 appliqué** via `01_customize` uniquement (commit `b4877674`) : 12 libellés courts — températures min/moy./max chambres-maison, 3 tendances, `clim_mode_local`, `ecs_temperature_ballon_securisee`, 4 scripts volets (le détail fait foi dans le runtime `01_customize/*.yaml`). Tendances et `clim_mode_local` en **`friendly_name` seul** : icônes dynamiques d'état **non écrasées**. **Validation visuelle Android Auto effectuée** le 2026-06-18 (terrain, écran Android Auto) : libellés lisibles et exploitables (volets, états principaux, températures et tendances) et grandes icônes de tendance validées. **Lot 2 livré techniquement** : `alarm_control_panel.alarme_maison` → « Alarme », `sensor.programme_chauffage` → « Chauffage » — en 2 étapes, verrouillage Lovelace préalable (commit `f7e41c05`) puis `friendly_name` court via `01_customize` sans `icon` (commit `01d5e057`). **Validation visuelle Android Auto effectuée** le 2026-06-18 (terrain, en voiture) : bloc Chauffage lisible, état chauffage « Eco » ; Alarme présente ; Clim « Désactivé », ECS « 35.6 », Ouvertures/Mouvement visibles ; aucun changement runtime. `binary_sensor.ouvertures_maison` et `binary_sensor.mouvement_maison` : catégorie A (déjà courts), sans changement. **Lot 3 livré techniquement** : `binary_sensor.panne_secteur_en_cours` → « Panne secteur » via `01_customize` sans `icon` (commit `f0b916e5`) ; aucun verrouillage Lovelace requis (entité sans affichage Lovelace). **En attente de validation visuelle Android Auto** (post-reload ; aucune capture terrain fournie à ce jour). Bluetti / UPS écartés d'Android Auto (diagnostic, hors surface conduite). | [`../architecture/00_structure_includes/01_customize.md`](../architecture/00_structure_includes/01_customize.md) § *friendly_name & surfaces d'affichage* |
