...
|---|---|---|
| Contrats `.md` (récursif) | **291** | `find 00_documentation_arsenal/contrats -name '*.md' \| wc -l` |
| Doctrines transversales | **12** | `ls 00_documentation_arsenal/architecture/03_doctrines/*.md \| grep -v README \| wc -l` |
| Checkers | **81** | `ls scripts/arsenal_contracts/check_*.py \| wc -l` |
| Workflows (total) | **85** | `ls .github/workflows/*.yml \| wc -l` |
| Workflows `contracts_*` (préfixe) | **79** | `ls .github/workflows/contracts_*.yml \| wc -l` |
...
| Couplage checker ↔ workflow à checker | **1:1** (81 ↔ 81) | aucun checker orphelin, aucun workflow à checker sans checker (confronté par `check_ci_coverage_registry.py`) |
