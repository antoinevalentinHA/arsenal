...
### `00_index.md`
- **Rôle** : inventaire réel `/homeassistant/button_card_templates/socle/` + correspondance **YAML ↔ docs**.

### `00_synthese.md`
- **Rôle** : synthèse de repérage (socles, rattachements TPL, axes de variations).
...
- **Socles** :
  ...
  - `socle_diagnostic_compact_readonly_72` *(72px capteur+label, read-only strict ; hérite de `socle_diagnostic_compact`)*
...
- (via `09_diagnostic.md`) `socle_diagnostic_compact_readonly_72` *(peut viser TPL-03)*
...
- (via `09_diagnostic.md`) `socle_diagnostic_compact_readonly_72` *(peut viser TPL-05)*
...
- **72px** : majorité des tuiles (KPI/Status/Action/Decision/Etat/Info/Diagnostic compact)
...
- **Read-only strict (tap/hold/double = none)** :
  ...
  - status read-only (72, compact, label*, decision_72, info_72)
  - diagnostic XL (`socle_diagnostic`)
  - diagnostic compact read-only (`socle_diagnostic_compact_readonly_72`)
  - `socle_badge_42` *(aucune action définie par le socle)*
  - `socle_header_base` *(actions neutralisées)*
- **More-info** :
...
- **Badge / header**
  - `socle_badge_42` : `background-color` via `variables.bg_color` (fallback transparent)
  - `socle_header_base` : `background: none`
- **Diagnostic** :
  ...
  - `socle_diagnostic_compact_readonly_72` (emphase state, actions neutralisées)
