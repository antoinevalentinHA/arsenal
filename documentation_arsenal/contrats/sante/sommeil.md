...
L'automation de consolidation répond à deux types d'événements complémentaires :

1. **Transition** : passage à ON de `binary_sensor.sommeil_donnees_exploitables`
   avec stabilisation 5 minutes — couvre le cas où Withings publie ses données
   dans la fenêtre autorisée.

2. **État déjà ON** : triggers horaires à **06:00** et **09:00** — couvrent le cas
   où le binary_sensor est déjà ON à l'ouverture de la fenêtre (données publiées
   la nuit, avant 06:00).

> Le temps ne déclenche pas à la place de la donnée.
> Il détecte un état déjà présent que la transition n'a pas pu capturer.

Les conditions (fenêtre, idempotence, bornes) s'appliquent identiquement
aux deux types de déclenchement.
...
| v1.3    | 2026-05-04 | Idempotence YYYY-MM-DD strict. Couche 1b : "signal d'orchestration exclusivement". |
| v1.4    | 2026-05-05 | Déclencheur dual : transition ON + horaires 06:00/09:00 pour état déjà ON. Le temps détecte, la donnée déclenche. |
