...
- **Not an install to copy.** Entities, IPs, MQTT topics, devices and domain choices are specific to one house. What is reusable is the patterns, the invariants and the method. The measurement and observability layers transpose cheaply — they are instantiated per location through an identifier suffix — but the decision domains are wired to this house's heating topology, appliances and room layout. Carrying them to another house is bounded rewriting, not a copy: what travels is the method and the invariants, to be studied and re-derived.
